/**
 * lib/mcp/tools/index.ts — SigRank MCP tool definitions and dispatcher.
 *
 * Extracted from app/api/mcp/route.ts as Phase 2 of the MCP structural
 * renovation. Contains the 15 read-only SigRank tool definitions, helper
 * functions, and the callTool dispatcher. Transport-level concerns (protocol
 * dispatch, observability, Exchange compatibility bridge) remain in the route.
 */
...
export const TOOLS = [
  {
    ...
    outputSchema: {
      ...
      properties: {
        ...
        yield_: { type: ["number", "null"], description: "Yield (Υ) = (cache_read × output) / input². Headline cascade efficiency. Null when input is zero." },
        leverage: { type: ["number", "null"], description: "Leverage = cache_read / input. Reusable context amplification. Null when input is zero." },
        velocity: { type: ["number", "null"], description: "Velocity = output / input. Output per unit of input. Null when input is zero." },
...
export function parsePillars(args: Record<string, unknown>): { input: number; output: number; cacheCreate: number; cacheRead: number } | { error: string } {
...
export function metricDelta(curr: number | null, sim: number | null) {
...
export function shareable(toolName: string, params: Record<string, unknown>, summary: string): { share_url: string; share_text: string } {
