...
type SignaWeights = { comp: number; sd: number; pc: number; ct: number; tt: number }
...
type ClassThreshold = { class: string; compMin: number; signaMin: number | null }
...
function validBuckets(v: unknown): v is DepthBucket[] {
  ...
  return v.every(
    (b) => Array.isArray(b) && b.length === 2 && isFiniteNumber(b[0]) && isFiniteNumber(b[1]),
  )
}
...
function loadEnvBlob(): Record<string, unknown> {
  ...
  try {
    ...
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      console.warn('[scoring/secret-config] SIGRANK_RULESET is not a JSON object — using placeholders')
      return {}
    }
    return parsed as Record<string, unknown>
  } catch {
    console.warn('[scoring/secret-config] SIGRANK_RULESET failed to parse — using placeholders')
    return {}
  }
}
...
function pick<T>(key: string, validate: (v: unknown) => v is T, fallback: T): T {
  if (Object.prototype.hasOwnProperty.call(ENV_BLOB, key)) {
    const candidate = ENV_BLOB[key]
    if (validate(candidate)) return candidate
    console.warn(`[scoring/secret-config] SIGRANK_RULESET.${key} invalid — using placeholder`)
  }
  return fallback
}
...
export const RS06_ANTI_GAMING: AntiGaming = pick(
  'RS06_ANTI_GAMING',
  (v): v is AntiGaming =>
    typeof v === 'object' && v !== null && typeof (v as Record<string, unknown>).enabled === 'boolean',
