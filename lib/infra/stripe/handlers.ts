...
import { tierForPrice } from "@/lib/infra/stripe/server";
import type { SubscriptionStatus, SubscriptionRecord } from "@/lib/infra/stripe/tier";
import {
  applyRewardsForOperator,
  recomputeSupporterTier,
} from "@/lib/infra/stripe/rewards";
...
export async function handleCheckoutCompleted(
  ...
  if (session.mode === "payment" && session.metadata?.kind === "donation") {
    ...
    return ok(
      `donation completed operator=${operatorId ?? "anon"} amount=${session.amount_total ?? "?"}`,
    );
  }
  ...
  if (session.mode === "subscription") {
    ...
    if (sb) {
      try {
        ...
        console.error(
          "[webhook] checkout subscription customer link failed",
          err,
        );
      }
    }
...
export async function handleSubscriptionUpsert(
  ...
  return ok(
    `subscription ${sub.id} (${sub.status}) synced operator=${operatorId}`,
  );
}
...
export async function handleInvoicePaid(
  ...
  const subId = typeof invSub === "string" ? invSub : (invSub?.id ?? null);
