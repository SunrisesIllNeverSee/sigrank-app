...
const fmtLev = (v: number) => (v >= 1000 ? `${(v / 1000).toFixed(1)}K×` : `${v.toFixed(0)}×`)
 ...
 */
export function deriveFacts(me: LeaderboardRow, foe: LeaderboardRow): OperatorFact[] {
  const c = me.snapshot.cascade
  const f = foe.snapshot.cascade
  if (!c) return []

  const facts: OperatorFact[] = []
  const push = (polarity: FactPolarity, label: string, detail: string, weight: number) =>
  ...
  push(reg.key === 'transient' || reg.key === 'stateless' ? 'down' : 'up', 'Regime', reg.word, 1.5)
  ...
  if (c.nonCompounding) {
    ...
    // ── Leverage (absolute) ──
    if (c.leverage >= B.levClosed) push('up', 'Leverage', `closed loop · ${fmtLev(c.leverage)}`, 4)
    else if (c.leverage >= B.levDeep) push('up', 'Leverage', `deep reuse · ${fmtLev(c.leverage)}`, 2.5)
    ...
    if (f && !f.nonCompounding && c.yield_ !== f.yield_) {
      ...
      if (ratio >= 1.15) push('up', 'Υ Yield', `leads ${ratio >= 10 ? fmtK(ratio) : ratio.toFixed(1)}× · ${fmtK(c.yield_)}`, 3.5)
      else if (ratio <= 0.87) push('down', 'Υ Yield', `trails · ${fmtK(c.yield_)}`, 3)
    }
  }

  // ── Velocity (absolute) ──
  if (c.velocity >= B.velFast) push('up', 'Velocity', `fast · ${c.velocity.toFixed(1)} o/i`, 2.5)
  else if (c.velocity < B.velSlow) push('down', 'Velocity', `slow · ${c.velocity.toFixed(2)} o/i`, 1.5)

  // ── Cost (absolute, lower-is-better) ──
  if (c.costPerMillion <= B.costLeader) push('up', 'Cost', `cost leader · $${c.costPerMillion.toFixed(2)}/1M`, 3)
  else if (c.costPerMillion >= B.costHeavy) push('down', 'Cost', `cost heavy · $${c.costPerMillion.toFixed(2)}/1M`, 2.5)

  // ── Efficiency (absolute) ──
  if (c.efficiency >= B.effStrong) push('up', 'Efficacy', `strong · ${c.efficiency.toFixed(2)}`, 2)

  // ── Scale (absolute, from raw total) ──
  const t = me.telemetry
  const total = t ? t.fresh_input + t.output + t.cache_read + t.cache_create : 0
  if (total > 0) {
    if (c.scaleV >= 9) push('up', 'Scale', `heavyweight · ${fmtK(total)} tok`, 2)
    else if (c.scaleV > 0 && c.scaleV < 6) push('flat', 'Scale', `lightweight · ${fmtK(total)} tok`, 1)
  }
