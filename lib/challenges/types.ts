...
export type ChallengeFormat = 'throwdown' | 'signal_drop' | 'bracket_match' | 'circle_war'
export type ChallengeStatus = 'pending' | 'active' | 'complete' | 'expired' | 'cancelled'
...
export function computeComposite(p: SignalPillars): number {
  return Math.round((p.density * 0.30 + p.clarity * 0.20 + p.fidelity * 0.20 + p.brevity * 0.15 + p.impact * 0.15) * 100) / 100
}
