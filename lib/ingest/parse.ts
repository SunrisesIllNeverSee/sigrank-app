...
function accumulateEntry(
  ...
  acc.cacheCreate += safeInt(
    e.cacheCreationTokens ??
      e.cache_creation_input_tokens ??
      e.cache_create ??
      0,
  );
  acc.cacheRead += safeInt(
    e.cacheReadTokens ?? e.cache_read_input_tokens ?? e.cache_read ?? 0,
  );
  acc.cost += Number(e.costUSD ?? e.totalCost ?? e.cost ?? 0) || 0;
}

function parseCcusage(d: unknown): {
  pillars: RawPillars;
  costUsd: number | null;
} {
...
function extractCodexTotals(d: unknown): {
  ...
  function addEntry(e: unknown): void {
    ...
    acc.cached += safeInt(
      obj.cached_input_tokens ?? obj.cachedInputTokens ?? 0,
    );
    acc.output += safeInt(obj.output_tokens ?? obj.outputTokens ?? 0);
    acc.reasoning += safeInt(
      obj.reasoning_output_tokens ?? obj.reasoningOutputTokens ?? 0,
    );
  ...
  if (Array.isArray(d)) {
    ...
    for (const key of [
      ...
      if (v && typeof v === "object") {
        Object.values(v).forEach(addEntry);
        return acc;
      }
    }
    addEntry(d);
 ...
 */
export function totalMismatchCaveat(
  p: RawPillars,
...
export function ingestMeta(
  ...
  const candidate = jsonStartMatch
    ? t.slice(t.indexOf(jsonStartMatch[0]))
    : null;
  const fixed =
