...
const k = (n: number) =>
  n >= 1000 ? `${(n / 1000).toLocaleString('en-US', { maximumFractionDigits: 1 })}K` : n.toFixed(2)
...
export function recordValue(row: LeaderboardRow, canonId: string): string {
  ...
  switch (canonId) {
    ...
    case 'Y.05':
      return c && !c.nonCompounding && c.dev10x !== null ? c.dev10x.toFixed(2) : '—'
    case 'Y.06':
    ...
    case 'T.05':
      return fmtTokens(t.fresh_input + t.output + t.cache_read + t.cache_create)
