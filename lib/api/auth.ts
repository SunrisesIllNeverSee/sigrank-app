...
import { getSessionOperator, type SessionOperator } from '@/lib/supabase/auth-server'
...
export async function resolveAuth(
  ...
  if (signature) {
    ...
    if (!deviceId) {
      return reject('signature_invalid', 'device_id required for signature auth')
    }
    ...
    if (device.trust_status !== 'trusted') {
      return reject('signature_invalid', `Device trust_status=${device.trust_status}`)
    }
...
function reject(reason: string, detail: string): AuthReject {
  return { ok: false, status: 401, body: { error: 'unauthorized', reason, detail } }
}
