...
export default async function MethodologyPage() {
  ...
  const topDecileStart =
    yields.length > 0
      ? yields[Math.floor(yields.length * 0.9)]
      : 0

  // Class tier distribution
  const transmitterCount = entries.filter((e) => e.signalClass === 'TRANSMITTER').length
  const transmitterPct =
    operatorCount > 0 ? Math.round((transmitterCount / operatorCount) * 100) : 0
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8 py-2">
      ...
      <WaveHero
        ...
        subtitle={
          <>
            The canonical source for AI operator token-efficiency data. Ranked by{' '}
            <strong className="text-text-primary">Υ Yield</strong> — the architecture
            of the cascade, not raw spend.
          </>
        }
      />
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          <p className="text-base text-text-secondary">
            As of <strong className="text-text-primary">{monthYear}</strong>, the SigRank
            Index ranks <strong className="text-text-primary">{operatorCount}</strong>{' '}
            operators across <strong className="text-text-primary">{platformCount}</strong>{' '}
            platforms.
          </p>
          {topEntry && (
            <p className="text-base text-text-secondary">
              The top-ranked operator{' '}
              <strong className="text-text-primary">{topEntry.anonId}</strong> achieves a
              yield of <strong className="text-gold">Υ {fmtYield(topYield)}</strong>.
            </p>
          )}
          <p className="text-base text-text-secondary">
            The <strong className="text-text-primary">median</strong> operator scores{' '}
            <strong className="text-text-primary">Υ {fmtYield(medianYield)}</strong>; the
            top decile starts at{' '}
            <strong className="text-text-primary">Υ {fmtYield(topDecileStart)}</strong>.
          </p>
          <p className="text-base text-text-secondary">
            Operators in the <strong className="text-text-primary">TRANSMITTER</strong>{' '}
            class tier represent the top{' '}
            <strong className="text-text-primary">{transmitterPct}%</strong> of the board.
          </p>
          <p className="text-base text-text-secondary">
            Across all ranked operators,{' '}
            <strong className="text-text-primary">{cachePct}%</strong> of input tokens are
            served from cache on average.
          </p>
        </div>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-base text-text-secondary">
          ...
          . It measures the architecture of an operator&rsquo;s token cascade — whether
          signal is compounding (high cache reuse × high output per fresh input) or tokens
          are burned (low cache, low output). Volume is noise; yield is signal.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-2 text-base text-text-secondary">
          <li>
            <strong className="text-text-primary">Inputs:</strong> on-device token counts
            (fresh input, output, cache_read, cache_create) per session per platform.
          </li>
          <li>
            <strong className="text-text-primary">Verification:</strong> each snapshot is
            ed25519-signed and verified server-side; replay and plausibility guards apply.
          </li>
          <li>
            <strong className="text-text-primary">Windows:</strong> operators are ranked
            over 7-day, 30-day, 90-day, and all-time cohorts.
          </li>
          <li>
            <strong className="text-text-primary">Privacy:</strong> token counts only —
            message content is never transmitted, read, or stored.
          </li>
          <li>
            <strong className="text-text-primary">Scoring:</strong> the yield metric Υ is
            computed from the four token pillars via a cascade model. The composite SIGNA
            rate blends Υ with signal-force and drift components. Proprietary weights
            (RS.xx) govern the composite and remain server-side.
          </li>
        </ul>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-base text-text-secondary">
          The Index updates continuously as operators submit signed snapshots. Public
          top-N data is available at{' '}
          <code className="rounded bg-bg-elevated px-1.5 py-0.5 font-mono text-sm text-gold">
            /api/v1/leaderboard
          </code>
          .
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-base text-text-secondary">
          ...
          . Attribution is the citation mechanism — reuse requires credit, which turns
          reuse into citations. The source code is separately licensed under MIT.
        </p>
        <p className="text-sm text-text-muted">
          Cite as: &ldquo;SigRank Index, {monthYear}. signalaf.com/methodology.&rdquo;
        </p>
      </section>
        ...
        <dl className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <dd className="text-base text-text-secondary">
              ...
              token-cascade efficiency (Υ = cache_read × output / input²), computed from
              privacy-preserving, on-device, cryptographically-signed token-telemetry
              snapshots.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            ...
            <dd className="text-base text-text-secondary">
              Each operator runs an on-device scanner that reads four token pillars
              locally. The yield metric Υ measures the architecture of the token cascade —
              whether signal is compounding or tokens are burned. No message content is
              ever read or transmitted.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            <dt className="font-semibold text-text-primary">Is the data private?</dt>
            <dd className="text-base text-text-secondary">
              Yes. The scanner reads token counts and content lengths only — never the
              words of your prompts. Only the resulting numeric scores, signed with
              ed25519, leave your device.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            <dt className="font-semibold text-text-primary">
