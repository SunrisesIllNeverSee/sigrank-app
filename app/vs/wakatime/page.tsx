...
const COMPARE_ROWS: { feature: string; wakatime: string; sigrank: string }[] = [
  { feature: 'Primary unit of measurement', wakatime: 'Time (hours/minutes)', sigrank: 'Token cascade efficiency (Υ Yield)' },
  { feature: 'Tracks AI coding specifically', wakatime: 'No (general coding time)', sigrank: 'Yes (AI operator scoring)' },
  { feature: 'Token pillar breakdown (input/output/cache-read/cache-write)', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Cache reuse measurement', wakatime: 'No', sigrank: 'Yes (cache hit rate, Leverage)' },
  { feature: 'Compression ratio (output per input)', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Signal-to-noise ratio', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Velocity (tokens per unit time)', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Class tier (IGNITER → TRANSMITTER)', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Global operator leaderboard', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Operator profiles + head-to-head compare', wakatime: 'No', sigrank: 'Yes' },
  { feature: 'Platform-neutral (Cursor, Copilot, Claude, 15+)', wakatime: 'Yes (editors)', sigrank: 'Yes (AI tools)' },
  { feature: 'Privacy-preserving (no prompt content read)', wakatime: 'Yes', sigrank: 'Yes (token counts only)' },
]
  ...
  return (
    <div className="mx-auto flex max-w-4xl flex-col gap-8 py-2">
      ...
      <WaveHero
        ...
        subtitle={
          <>
            WakaTime tracks the <span className="text-gold">clock</span>. SigRank
            tracks the <span className="text-gold">cascade</span>. Time ≠ signal
            — an hour with good cache reuse beats 10 hours of burning input
            tokens.
          </>
        }
      />
      ...
      <section className="flex flex-col gap-4">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          <strong className="text-text-primary">token cascade efficiency</strong>.
          An hour with good cache reuse and tight prompts can produce more signal
          than ten hours of re-sending bloated context. WakaTime would rank both
          sessions by duration. SigRank ranks them by{' '}
            ...
            <tbody>
              {COMPARE_ROWS.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 text-text-secondary">{r.wakatime}</td>
      ...
      <section className="flex flex-col gap-4">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          keystrokes. In AI coding, the model does the keystrokes — so work ≈ how
          efficiently you steered the model&apos;s token flow. Two operators, same
          hour:
        </p>
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          Operator A higher — 10× the signal per token. In a world where the model
          writes the code, the second ranking is the one that reflects skill.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3 rounded-lg border border-bg-border bg-bg-surface p-6">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          WakaTime told you how long you sat there. SigRank tells you how well you
          drove. Install the CLI, submit a signed snapshot, and see your Υ Yield,
      ...
      <section className="mt-4 border-t border-bg-border-subtle pt-6">
        <p className="font-sans text-sm text-text-muted">
          ...
          {' · '}
          <Link href="/metrics/velocity" className="text-gold underline underline-offset-2">
            Velocity
          </Link>
          {' · '}
