    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-8 py-2">
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          <strong className="text-text-primary">cache_read</strong> rewards
          context reuse — the cached tokens you carry forward from prior turns.{" "}
          <strong className="text-text-primary">output</strong> rewards
          productive generation — the tokens the model gives back.{" "}
          <strong className="text-text-primary">input²</strong> penalizes fresh
          input — and the square means the penalty is non-linear. Double your
          input and you quarter your yield, all else equal.
        </p>
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          The four token pillars are:{" "}
          <strong className="text-text-primary">input</strong> (tokens you
          send), <strong className="text-text-primary">output</strong> (tokens
          the model generates),{" "}
          <strong className="text-text-primary">cache_read</strong> (cached
          tokens reused from prior context), and{" "}
          <strong className="text-text-primary">cache_write</strong> (new tokens
          written to cache). Yield uses three of the four — cache_write is
          captured indirectly through future cache_read.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          token cascade — whether signal is compounding or tokens are being
          burned. An operator who sends 10,000 fresh input tokens, reuses 50,000
          from cache, and gets back 8,000 output tokens is running an efficient
          cascade. An operator who sends 50,000 fresh tokens, reuses nothing,
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          Yield is the single number that captures the full cascade. Other
          metrics isolate one dimension — compression ratio measures output per
          input, cache hit rate measures context reuse, leverage measures cached
          amplification. Yield blends all of them. An operator can have a high
          compression ratio but low yield (if they never reuse cache). An
          operator can have a high cache hit rate but low yield (if they produce
          little output). Yield is the only metric that rewards getting{" "}
          <em>all three</em> dimensions right simultaneously.
        </p>
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          The input² denominator is the key design choice. It makes yield
          sensitive to input economy in a way that linear ratios are not. This
          reflects a real asymmetry: fresh input tokens are the most expensive
          part of the cascade (they cost full price and must be processed from
          scratch), so the metric penalizes waste there disproportionately.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            Your cascade is compounding. You reuse cached context efficiently,
            send minimal fresh input, and the model produces substantial output.
            You&rsquo;re building on prior turns rather than re-explaining from
            scratch. This is the signature of a{" "}
            <strong className="text-text-primary">TRANSMITTER</strong> — an
            operator whose signal accumulates across a session.
          </p>
        </div>
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            Your cascade is burning tokens. You send large fresh inputs, reuse
            little or no cache, and get back relatively little output. Each turn
            starts from scratch. This is the signature of an{" "}
            <strong className="text-text-primary">IGNITER</strong> — an operator
            who hasn&rsquo;t yet built the context discipline that makes signal
            compound.
          </p>
        </div>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            High cache reuse, moderate output, low fresh input. Signal is
            compounding.
          </p>
        </div>
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            No cache reuse kills yield — the cache_read term zeroes the
            numerator regardless of output. Every turn starts from scratch.
          </p>
        </div>
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          <p className="font-mono text-xs uppercase tracking-[0.14em] text-text-dim">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            Good cache reuse and output, but the large fresh input crushes yield
            via the squared denominator. Trimming input from 20k to 5k would
            raise yield to 12 — a 16× improvement from a 4× input reduction.
          </p>
        </div>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-2 font-sans text-sm leading-relaxed text-text-secondary">
          <li>
            <strong className="text-text-primary">IGNITER</strong> — Low yield.
            Just starting out; context discipline not yet developed. Every turn
            is a fresh start.
          </li>
          <li>
            <strong className="text-text-primary">SEEKER</strong> — Improving
            yield. Beginning to reuse cache and trim input, but output is still
            modest.
          </li>
          <li>
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          Exact yield thresholds are calibrated from the live leaderboard and
          shift as the operator population grows. The tier you land in reflects
          your cascade architecture relative to the current field.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-3 font-sans text-sm leading-relaxed text-text-secondary">
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">1. Maximize cache reuse.</strong> Keep
            your context stable across turns. Don&rsquo;t restart sessions
            unnecessarily. Prompt caching rewards continuity — the longer your
            cached prefix, the more cache_read accumulates and the higher your
            numerator climbs.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">
              2. Trim fresh input aggressively.
            </strong>{" "}
            The input² denominator means every token you cut from fresh input
            has a squared effect on yield. Remove redundant context, avoid
            re-pasting code that&rsquo;s already in cache, and use file
            references instead of inline blocks when possible.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">
              3. Ask for substantive output.
            </strong>{" "}
            Request deliverables — code, analysis, explanations — rather than
            yes/no confirmations. More output tokens means a higher numerator. A
            turn that produces 5,000 output tokens contributes far more to yield
            than one that produces 50.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">4. Use structured prefixes.</strong>{" "}
            Stable, structured context at the top of your prompt (project
            conventions, file layout, coding standards) gets cached and reused.
            Chaotic, re-ordered context breaks the cache and forces fresh
            processing every turn.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">5. Build on prior turns.</strong>{" "}
            Reference earlier output instead of re-explaining. &ldquo;Refactor
            the function you just wrote&rdquo; leverages cache; &ldquo;here is a
            function, refactor it&rdquo; burns fresh input. Conversation
            continuity is yield continuity.
          </li>
        </ul>
      </section>
        ...
        <dl className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              ...
              metric. It measures token cascade efficiency — how well an AI
              operator converts fresh input into output, amplified by cached
              context. High yield means signal is compounding; low yield means
              tokens are burned.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Yield captures all four token pillars in a single number. It
              rewards cache reuse, output production, and input economy
              simultaneously. Volume alone is noise — yield measures the
          ...
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Three levers: increase cache_read (stable context, structured
              prefixes), increase output (ask for substantive deliverables), and
              decrease input (trim redundant context). The input² term means
              cutting input has a squared effect — small reductions produce
              large yield gains.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              No. SigRank scores the operator, not the model. Yield is computed
              from four token counts that every platform reports. The same
              operator on Claude, ChatGPT, Gemini, or Cursor produces comparable
              yield scores because the metric measures how you drive the AI, not
              which AI you drive.
            </dd>
          </div>
        </dl>
