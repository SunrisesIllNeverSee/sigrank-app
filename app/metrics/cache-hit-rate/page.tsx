...
export const metadata: Metadata = withOG({
  ...
  description:
    "Cache hit rate measures how well you reuse cached context in AI coding. Learn why it is the highest-leverage metric for token-cascade efficiency and how to improve your reuse.",
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-8 py-2">
      <JsonLd
        data={[
          ...
          faqPage([
            ...
            {
              question:
                "What is the impact of cache hit rate on cost and latency?",
              answer:
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          (creating new cache). A hit rate of 0.9 means 90% of your cache
          activity is reuse — you&rsquo;re efficiently building on prior
          context. A hit rate of 0.1 means 90% is writing cache that&rsquo;s
          never read back — you&rsquo;re constantly establishing context that
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          ChatGPT, Gemini, Copilot, Cursor, and others. When you send a prompt,
          the platform stores the processed tokens (the{" "}
          <strong className="text-text-primary">cache_write</strong>). On
          subsequent turns, if your prompt begins with the same prefix, the
          platform reuses the stored tokens (the{" "}
          <strong className="text-text-primary">cache_read</strong>) instead of
          reprocessing them from scratch.
        </p>
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          The economics are dramatic. Cache reads typically cost{" "}
          <strong className="text-text-primary">10x less</strong> than fresh
          input tokens and process{" "}
          <strong className="text-text-primary">significantly faster</strong> —
          the model reads from memory rather than recomputing attention over the
          full context. An operator with a 90% cache hit rate pays a fraction of
          what a 10% hit-rate operator pays for the same total context size.
        </p>
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          The catch: cache breaks when context changes. If you reorder your
          prompt, rewrite the prefix, or switch topics mid-session, the cached
          prefix no longer matches and the platform falls back to fresh
          processing. Cache hit rate is fundamentally a measure of{" "}
          <strong className="text-text-primary">context stability</strong>.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          ...
          cascade. Every cache hit replaces an expensive fresh-input token with
          a cheap cached one. Improving cache hit rate cascades into every other
          metric:
        </p>
        <ul className="flex flex-col gap-1.5 font-sans text-sm leading-relaxed text-text-secondary">
          <li>
            ...
            cache_read is in the yield numerator. More cache reads, higher
            yield.
          </li>
          <li>
            <strong className="text-text-primary">SNR goes up</strong> —
            cache_read is signal. More signal, higher SNR.
          </li>
          <li>
          ...
          <li>
            <strong className="text-text-primary">Cost goes down</strong> —
            cache reads cost a fraction of fresh input.
          </li>
          <li>
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          No other metric has this many downstream effects. Compression ratio
          only affects yield through one term. Velocity is secondary. Cache hit
          rate is the multiplier that amplifies everything else — which is why
          it&rsquo;s the first metric to optimize when improving your cascade.
        </p>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-3 font-sans text-sm leading-relaxed text-text-secondary">
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">1. Keep context stable.</strong> The
            cache matches on prefix — if your prompt starts the same way each
            turn, the cache hits. Don&rsquo;t reorder your context, rewrite the
            opening, or shuffle file order between turns. Stability is the
            single most important factor.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">2. Use structured prefixes.</strong>{" "}
            Put stable, structured content at the top of your context: project
            ...
            subsequent turn. Put variable content (the specific question or
            task) at the bottom.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            ...
            Don&rsquo;t restart sessions unnecessarily. The cached prefix from
            turn 5 is free signal on turn 50. Each session restart forces a new
            cache_write with no prior cache_read — a hit rate of zero until the
            cache builds again.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">
              4. Avoid mid-session context switches.
            </strong>{" "}
            Jumping between unrelated topics in one session dilutes the cache.
            Each topic switch changes the prefix and breaks the cache. Group
            related work into focused sessions where the cache stays relevant
            throughout.
          </li>
          <li className="rounded-lg border border-bg-border-subtle bg-bg-surface p-4">
            <strong className="text-gold">
              5. Reference, don&rsquo;t re-paste.
            </strong>{" "}
            If a file is already in context, reference it by name. Re-pasting
            its contents changes the prompt prefix and can break the cache.
            &ldquo;In the auth module from earlier&rdquo; preserves the cache;
            pasting auth.ts again may invalidate it.
          </li>
        </ul>
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            Cache reads typically cost{" "}
            <strong className="text-text-primary">10x less</strong> than fresh
            input tokens on platforms that support prompt caching. An operator
            processing 100,000 tokens per session with a 90% cache hit rate pays
            for ~10,000 fresh tokens and ~90,000 cached tokens — a fraction of
            the cost of processing all 100,000 fresh. The same operator at 10%
            hit rate pays full price on 90,000 tokens.
          </p>
        </div>
        <div className="rounded-lg border border-bg-border bg-bg-surface p-5">
          <p className="font-mono text-xs uppercase tracking-[0.14em] text-gold">
          ...
          <p className="mt-2 font-sans text-sm leading-relaxed text-text-secondary">
            Cached tokens are served from memory — the model reads the
            precomputed attention state rather than recomputing it. This makes
            cached turns
            <strong className="text-text-primary"> significantly faster</strong>
            , especially for long contexts. A 50,000-token cached prefix
            processes in a fraction of the time it would take to recompute from
            scratch. High cache hit rate means faster responses, not just
            cheaper ones.
          </p>
        </div>
      </section>
        ...
        <dl className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Cache Hit Rate = cache_read / (cache_read + cache_write). It
              measures the fraction of cache operations that are reads (reusing
              prior context) versus writes (creating new cache). A hit rate of
              0.9 means 90% of your cache activity is reuse.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            <dt className="font-semibold text-text-primary">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Prompt caching stores processed tokens from prior turns and reuses
              them in subsequent turns at reduced cost and latency. Instead of
              reprocessing your full context from scratch, the model reads the
              cached prefix and only processes new input. Cache reads are
              typically 10x cheaper and faster than fresh input.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            <dt className="font-semibold text-text-primary">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Cache reuse is the cheapest, fastest source of signal. Every cache
              hit replaces an expensive fresh-input token with a cheap cached
              one. Improving cache hit rate increases yield, SNR, and leverage
              while reducing cost and latency — no other metric has this many
              downstream effects.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Keep context stable across turns, use structured prefixes at the
              top of your prompts, maintain conversation continuity, avoid
              mid-session context switches, and reference files instead of
              re-pasting them. Cache breaks when context changes, so stability
              is everything.
            </dd>
          </div>
          <div className="flex flex-col gap-1">
            ...
            <dd className="font-sans text-sm leading-relaxed text-text-secondary">
              Cache reads cost a fraction of fresh input and process
              significantly faster. A 90% cache hit rate means paying a fraction
              of the cost and getting faster responses compared to a 10% hit
              rate for the same total context. High cache hit rate is the single
