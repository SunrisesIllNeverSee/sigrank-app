...
export async function POST(req: NextRequest) {
  ...
  const { pillars, sessionsCount, turnsTotal, totalMessagesLifetime, accountAgeDays } = body

  if (!pillars || typeof pillars.input !== 'number' || typeof pillars.output !== 'number' ||
      typeof pillars.cacheCreate !== 'number' || typeof pillars.cacheRead !== 'number') {
    return NextResponse.json({ error: 'Missing or invalid pillars' }, { status: 400 })
  }
