...
export async function POST(req: NextRequest) {
  ...
  const { data: opRow } = await svc
    ...
    .maybeSingle()
  const customerId =
    opRow && typeof opRow.stripe_customer_id === 'string' ? opRow.stripe_customer_id : null
  ...
  if (customerId && stripe) {
    try {
      ...
      for (const sub of subs.data) {
        if (CANCELABLE.has(sub.status)) await stripe.subscriptions.cancel(sub.id)
      }
    } catch (err) {
  ...
  const { error: rpcErr } = await svc.rpc('delete_account', { p_operator_id: op.operatorId })
