...
export async function POST(req: NextRequest) {
  ...
  const newPublicKey = typeof body.new_public_key === 'string' ? body.new_public_key : ''
  const deviceLabel =
    typeof body.device_label === 'string' ? body.device_label.trim().slice(0, 120) : null

  if (!oldDeviceId || !UUID_RE.test(oldDeviceId)) {
    return NextResponse.json({ error: 'A valid device_id (uuid) for the old device is required.' }, { status: 400 })
  }
  if (!newPublicKey || !isValidAgentPublicKey(newPublicKey)) {
    return NextResponse.json({ error: 'A valid new_public_key (ed25519) is required.' }, { status: 400 })
  }
