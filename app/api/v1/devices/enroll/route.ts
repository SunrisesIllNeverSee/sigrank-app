...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ reason: 'bad_request', detail: 'Invalid JSON.' }, { status: 400 })
  }

  const code = typeof body.code === 'string' ? normalizeConnectCode(body.code) : ''
  ...
  const agentVersion = typeof body.agent_version === 'string' ? body.agent_version.trim().slice(0, 60) : null

  if (!code || !UUID_RE.test(deviceId) || !publicKey || !isValidAgentPublicKey(publicKey)) {
    return NextResponse.json(
      { reason: 'bad_request', detail: 'code, device_id (uuid), and a valid ed25519 public_key are required.' },
      { status: 400 },
    )
  }

  const svc = getSupabaseService()
  if (!svc) return NextResponse.json({ reason: 'persistence_unavailable' }, { status: 503 })
  ...
  if (error) {
    return NextResponse.json({ reason: 'enroll_failed', detail: error.message }, { status: 500 })
  }

  const row = Array.isArray(data)
    ? (data[0] as { status: string; operator_id: string | null; codename: string | null } | undefined)
    : null
  switch (row?.status) {
    case 'enrolled':
      return NextResponse.json(
    ...
    case 'device_already_enrolled':
      return NextResponse.json({ reason: 'device_already_enrolled' }, { status: 409 })
    default:
