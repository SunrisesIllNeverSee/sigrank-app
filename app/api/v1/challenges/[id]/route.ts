...
export async function GET(
  ...
  if (!sb) {
    return NextResponse.json({ error: 'db_not_configured', mock: true }, { status: 503 })
  }
...
export async function PATCH(
  ...
  if ([density, clarity, fidelity, brevity, impact].some((v) => v < 0 || v > 100)) {
    return NextResponse.json({ error: 'invalid_scores', detail: 'All scores must be 0–100' }, { status: 400 })
  }
  ...
  const { data: challenge } = await sb
    .from('challenges')
    .select('challenge_id, status, challenger_id, challenged_id, window_close, format')
    .eq('challenge_id', id)
    .single()
  ...
  if (challenge.status !== 'active') {
    return NextResponse.json({ error: 'challenge_not_active', detail: `Status is '${challenge.status}'` }, { status: 409 })
  }
  if (new Date() > new Date(challenge.window_close)) {
    return NextResponse.json({ error: 'window_closed', detail: 'Submission window has closed' }, { status: 409 })
  }
  ...
  if (subErr) {
    return NextResponse.json({ error: 'db_error', detail: subErr.message }, { status: 500 })
  }
  ...
  if (challenge.format === 'throwdown' && challenge.challenger_id && challenge.challenged_id) {
    const { data: subs } = await sb
      .from('challenge_submissions')
    ...
    if (subs && subs.length >= 2) {
      const challSub  = subs.find((s) => s.operator_id === challenge.challenger_id)
      const challdSub = subs.find((s) => s.operator_id === challenge.challenged_id)

      if (challSub && challdSub) {
        const challScore  = Number(challSub.composite_score)
        const challdScore = Number(challdSub.composite_score)
        const winnerId    = challScore >= challdScore
        ...
        const { data: w } = await sb
          .from('operators').select('codename').eq('operator_id', winnerId).single()
