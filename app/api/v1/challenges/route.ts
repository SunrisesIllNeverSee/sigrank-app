...
export async function POST(req: NextRequest) {
  ...
  if (!['throwdown', 'signal_drop', 'bracket_match', 'circle_war'].includes(format)) {
    return NextResponse.json({ error: 'invalid_format', detail: 'format must be throwdown|signal_drop|bracket_match|circle_war' }, { status: 400 })
  }

  const now = new Date()
  const windowClose = new Date(now.getTime() + windowHours * 60 * 60 * 1000)
  const challengeId = deterministicId('ch', challengerCodename, challengedCodename, now.toISOString().slice(0, 16))
  ...
  if (challengedCodename) {
    const { data: challenged } = await sb
      ...
      .single()
    if (!challenged) {
      return NextResponse.json({ error: 'operator_not_found', detail: `Challenged '${challengedCodename}' not found` }, { status: 404 })
    }
...
export async function GET(req: NextRequest) {
  ...
  if (codename) {
    ...
    const { data: op } = await sb
      ...
      .single()
    if (op) {
      q = q.or(`challenger_id.eq.${op.operator_id},challenged_id.eq.${op.operator_id}`)
    }
  }
