...
/**
 * logAudit — append-only audit_log write, getSupabaseServer()-guarded. With no
 ...
 *   3. Idempotency: insert into webhook_events ON CONFLICT(event_id) DO NOTHING;
 *      a conflict means we've already processed this event → ack 200.
 *   4. Dispatch to the handlers (all DB ops getSupabaseServer()-guarded).
 *
...
export async function POST(req: Request) {
  ...
  // ── Idempotency via webhook_events ON CONFLICT(event_id) DO NOTHING ───────
  const sb = getSupabaseServer()
  if (sb) {
    try {
      // Canonical webhook_events columns (schema.sql §14): event_type (NOT
      // 'type') + received_at + payload_json (NOT NULL).
      ...
      // ignoreDuplicates returns no rows for an existing event_id → already done.
  ...
  try {
    ...
    // Return 200 anyway so Stripe does not retry a non-recoverable handler error;
    // the failure is logged for follow-up.
    await logAudit('stripe_webhook_handler_error', {
      event_id: event.id,
      type: event.type,
    })
    return NextResponse.json({ received: true, handled: false, error: 'handler_error' })
  }
}
