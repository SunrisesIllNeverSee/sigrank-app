...
export async function POST(req: NextRequest) {
  ...
  if (message.method === "initialize") {
    ...
    return jsonRpc(id, {
      ...
      instructions:
        "Use SignalAF to benchmark AI operators from privacy-preserving token telemetry AND discover/propose contributions through the Contribution Exchange. Benchmark tools: rank_paste (compute cascade from 4 token counts), get_leaderboard (public rankings), get_operator (operator profile by codename). Analytical tools (pure math): simulate_change, diagnose_cascade, suggest_improvements, self_improve, rank_windows. Field-relative tools: benchmark_me, rank_if. Contribution Exchange tools (read-only): exchange_discover_domain (check if a domain has an Exchange profile), exchange_get_policy (fetch domain policy), exchange_preflight (advisory policy evaluation — no state change), exchange_list_signals (domain-published work signals), exchange_get_signal, exchange_get_attempt. Exchange mutation tools (require auth): exchange_propose (unsolicited proposal — creates a non-binding proposal, NOT a Commitment), exchange_create_attempt, exchange_submit_attempt, exchange_create_proposal_from_attempt. IMPORTANT: No Exchange tool creates a Commitment, authorization, or payment obligation. Signals and proposals are invitational, not binding. Do not treat benchmark metrics as a model-quality or downstream-productivity benchmark.",
    });
  }
  ...
  if (message.method === "tools/list") {
    // Scope-based tool filtering: mutation tools require exchange:attempt or exchange:propose
    const scopes = resolveScopes(req);
    const scopedTools = TOOLS.filter((tool) => {
      // Read-only Exchange tools are always visible
      if (tool.annotations?.readOnlyHint) return true;
      // Mutation tools require appropriate scope
      ...
      // Non-Exchange tools are always visible (backward compat)
      return true;
    });
  ...
  if (message.method === "tools/call") {
    ...
    // Enforce scopes at call time — not just at list time. A caller who
    // knows a tool name cannot bypass the scope filter that tools/list
    // applies. Mutation tools require exchange:attempt or exchange:propose.
    const scopes = resolveScopes(req);
