...
const TOOLS: Tool[] = [
  {
    name: "SigRank",
    what: "An operator-scoring platform that tracks the four token pillars (input, output, cache-read, cache-write), computes cascade efficiency (Υ Yield = cache_read × output / input²), and ranks operators on a live leaderboard with class tiers.",
    pros: [
  ...
  {
    name: "ccusage",
    what: "A CLI that reads Claude Code token logs locally and reports input, output, cache-read, and cache-write counts per session. Pure measurement — no scoring.",
    pros: [
  ...
  },
  {
    ...
    what: "A token-visualization tool that turns raw token counts into charts and breakdowns of input, output, cache-read, and cache-write. Bundled with SigRank.",
    pros: [
  ...
  {
    name: "Tiktoken",
    what: "OpenAI's tokenizer library that counts tokens for a given string and model encoding. A building block, not a tracker — it counts tokens in text, not across sessions.",
    pros: [
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-8 py-2">
      ...
      <section className="flex flex-col gap-4">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          Token tracking splits into two layers.{" "}
          <strong className="text-text-primary">Counting</strong> tells you how
          many tokens you sent and received — ccusage, Tiktoken, and Token
          Dashboard live here.{" "}
          <strong className="text-text-primary">Scoring</strong> tells you
          whether those tokens compounded or burned — and that is SigRank alone.
          Knowing you spent 500K tokens is useful. Knowing your Υ Yield puts you
          in the top decile is actionable. The best stack does both: count, then
          score.
        </p>
      ...
      <section className="flex flex-col gap-3 rounded-lg border border-gold/30 bg-gold/5 p-6">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          If you only need to <em>count</em> tokens, ccusage (for Claude Code)
          or Tiktoken (for OpenAI strings) are the simplest options. If you want
          to <em>visualize</em> them, Token Dashboard is clean. But if you want
          to know whether your tokens are{" "}
          <strong className="text-text-primary">compounding or burning</strong>{" "}
          — and where you rank against every other operator — SigRank is the
          only tool that scores and ranks. It bundles the counters, so you lose
