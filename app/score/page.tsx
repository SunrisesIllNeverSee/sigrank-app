    ...
    <main className="mx-auto max-w-3xl px-4 py-12 sm:py-16">
      ...
      <div className="flex flex-col gap-3 text-center">
        ...
        <p className="mx-auto max-w-xl font-sans text-sm leading-relaxed text-text-secondary">
          Install the agent. It reads your local AI session logs on-device, derives your cascade,
          and submits a signed snapshot to the board. No paste, no prompts read — only the four
          token counts leave your machine.
        </p>
      </div>
      ...
      <div className="mt-10 flex flex-col gap-3">
        <div className="flex items-center gap-3 rounded-lg border border-bg-border bg-bg-base px-4 py-3">
          <span className="font-mono text-xs font-semibold text-text-muted">1</span>
        ...
        <div className="flex items-center gap-3 rounded-lg border border-bg-border bg-bg-base px-4 py-3">
          <span className="font-mono text-xs font-semibold text-text-muted">2</span>
          <code className="flex-1 overflow-x-auto font-mono text-sm font-semibold text-text-accent">
            sigrank enroll
          </code>
        ...
        <div className="flex items-center gap-3 rounded-lg border border-bg-border bg-bg-base px-4 py-3">
          <span className="font-mono text-xs font-semibold text-text-muted">3</span>
          <code className="flex-1 overflow-x-auto font-mono text-sm font-semibold text-text-accent">
            sigrank submit
          </code>
          ...
          {PIPELINE.map((p) => (
            <div key={p.step} className="flex gap-4">
              ...
              <div className="flex flex-col gap-1">
                <h3 className="font-sans text-sm font-semibold text-text-primary">{p.title}</h3>
                <p className="font-sans text-sm leading-relaxed text-text-secondary">{p.body}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {PRIVACY_POINTS.map((p) => (
            <div key={p.t} className="flex flex-col gap-1">
              <h3 className="font-sans text-sm font-semibold text-text-primary">{p.t}</h3>
              <p className="font-sans text-sm leading-relaxed text-text-secondary">{p.b}</p>
            </div>
          ))}
        </div>
      ...
      <div className="mt-12 border-t border-bg-border-subtle pt-6">
        ...
        <p className="mt-1 font-sans text-xs leading-relaxed text-text-muted">
          The paste calculator is a backup — no account, no save, just the numbers. The agent path
          is how you compete.
        </p>
      </div>
    </main>
