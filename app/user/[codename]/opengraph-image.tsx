...
function radarSvg(rows: RadarRow[], svgSize: number) {
  ...
  const labelAt = (i: number): { left: number; top: number } => {
    ...
    return {
      left: cx + Math.cos(a) * (radius + 30),
      top: cy + Math.sin(a) * (radius + 26),
    };
  };
...
export default async function OperatorOG({
  ...
  if (!row) {
    return new ImageResponse(
      ...
      >
        ...
        <div
          style={{ display: "flex", fontSize: 36, opacity: 0.6, marginTop: 16 }}
        >
          Operator not found
        </div>
      </div>,
  ...
  const costStr =
    ranked && c.costPerMillion != null
      ? `$${c.costPerMillion.toFixed(2)}`
      : DASH;
  ...
  const totalT =
    inputT != null && outputT != null && cacheR != null && cacheW != null
      ? inputT + outputT + cacheR + cacheW
      : null;
  ...
  const rawLines: [string, string, string, string, string][] = [
    [
      "INPUT",
      inputT != null ? fmtTokens(inputT) : DASH,
      "IN",
      avgFmt(avgIn),
      C_BONE,
    ],
    [
      "OUTPUT",
      outputT != null ? fmtTokens(outputT) : DASH,
      "OUT",
      avgFmt(avgOut),
      C_GREEN,
    ],
    [
      "CACHE R",
      cacheR != null ? fmtTokens(cacheR) : DASH,
      "CR",
      avgFmt(avgCache),
      C_GREEN,
    ],
    [
      "CACHE W",
      cacheW != null ? fmtTokens(cacheW) : DASH,
      "CW",
      avgIn != null ? "0" : DOT,
      C_BONE,
    ],
    [
      "TOTAL",
      totalT != null ? fmtTokens(totalT) : DASH,
      "TOT",
      avgFmt(avgTotal),
      C_DIM,
    ],
  ...
  return new ImageResponse(
    ...
    >
      ...
      >
        ...
        >
          <div style={{ display: "flex", width: 120, color: C_GREEN }}>
            TELEMETRY
          </div>
          <div
            style={{ display: "flex", width: 195, justifyContent: "flex-end" }}
          >
            WELCOME OPERATOR
          </div>
          <div
        ...
        {rawLines.map(([label, you, glyph, avg, color]) =>
          printRow(`raw-${glyph}`, label, you, glyph, avg, color),
        )}
        {derivedLines.map(([label, you, glyph, avg, color]) =>
          printRow(`der-${glyph}`, label, you, glyph, avg, color),
        )}
        ...
        >
          <div style={{ display: "flex", width: 130, color: C_DIM }}>
            OP RATIO
          </div>
          <div
            style={{
