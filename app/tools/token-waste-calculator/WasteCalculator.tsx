...
const PILLARS = [
  {
    key: "input",
    label: "Input tokens",
    hint: "Fresh tokens sent to the model",
  },
  ...
  },
  {
    ...
    hint: "New tokens written to cache",
  },
] as const;
...
export function WasteCalculator() {
  ...
  const breakdown = [
    {
      label: "Excess fresh input",
      value: result.excessInput,
      hint: "Input beyond a 10:1 input-to-output ratio — likely over-specified prompts.",
    },
    {
      label: "Unreused cache writes",
      value: result.unreusedCacheWrite,
      hint: "Cache-write tokens never read back — context cached but not reused.",
    },
    {
      label: "Repeated context",
      value: result.repeatedContext,
      hint: "Cache-read tokens far exceeding what plausibly shaped output — re-sent boilerplate.",
    },
  ];
  ...
  return (
    <div className="rounded-xl border border-bg-border bg-bg-surface p-6">
      ...
      <p className="mt-1 font-sans text-sm leading-relaxed text-text-secondary">
        From <code className="font-mono text-text-primary">ccusage --json</code>{" "}
        or <code className="font-mono text-text-primary">npx sigrank me</code>.
        Token counts only.
      </p>
      ...
      <div className="mt-6 rounded-lg border border-bg-border bg-bg-elevated p-5">
        ...
        <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 font-mono text-xs text-text-secondary">
          ...
          <span>
            Total in:{" "}
            <span className="text-text-primary">{fmt(result.totalIn)}</span>
          </span>
        </div>
      </div>
      ...
      <p className="mt-5 font-sans text-xs leading-relaxed text-text-muted">
        This is a{" "}
        <strong className="text-text-primary">heuristic estimator</strong>, not
        a measurement. The categories use rough proxies (a 10:1 efficient
        input-to-output ceiling, cache-write vs cache-read reuse, a 20:1
        cache-read-to-output threshold). Real waste depends on your actual
        prompts and workflow. Use the numbers as a directional signal, not an
        audit. The Υ Yield metric is the rigorous counterpart.
      </p>
    </div>
  );
}
