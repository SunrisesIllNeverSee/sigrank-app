  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8 py-2">
      <JsonLd
        data={[
          breadcrumb([
            ...
            { name: 'Operator Class Checker', path: '/tools/operator-class-checker' },
          ]),
          faqPage(FAQS),
          {
      ...
      <WaveHero
        ...
        subtitle={
          <>
            Which tier are you? Enter a yield score — or four token pillars to compute one —
            and see your <span className="text-gold">class tier</span> on the ladder from
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          Every operator starts as an <strong className="text-text-primary">IGNITER</strong> —
          tokens burned for context, nothing yet compounding. The first promotion comes from
          caching, not from typing more. <strong className="text-text-primary">SEEKER</strong>{' '}
          operators have a working cascade but spend most input once.{' '}
          <strong className="text-text-primary">BUILDER</strong> operators compound: good cache
          reuse, dense output. <strong className="text-text-primary">TRANSMITTER</strong>{' '}
          operators turn cached context into a multiplier — every fresh input yields outsized
          output. The ladder measures the <em>architecture</em> of your cascade, not your raw
          spend.
        </p>
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          Thresholds shown here are approximate. The authoritative tier is computed server-side
          from ed25519-signed snapshots submitted via the SigRank CLI — see{' '}
