...
const PILLARS = [
  { key: 'input', label: 'Input tokens', hint: 'Fresh tokens you send to the model' },
  { key: 'output', label: 'Output tokens', hint: 'Tokens the model generates back' },
  { key: 'cacheRead', label: 'Cache-read tokens', hint: 'Cached tokens reused from prior context' },
  { key: 'cacheWrite', label: 'Cache-write tokens', hint: 'New tokens written to cache for reuse' },
] as const
...
function classForYield(y: number): { tier: string; blurb: string } {
  if (y >= 10) return { tier: 'TRANSMITTER', blurb: 'Signal compounds aggressively — cached context amplifies every fresh input into outsized output.' }
  if (y >= 2) return { tier: 'BUILDER', blurb: 'A productive cascade — good cache reuse and solid output per input. Compounding, not just burning.' }
  if (y >= 0.5) return { tier: 'SEEKER', blurb: 'A working cascade, but most input is spent once. Cache reuse and output density have clear headroom.' }
  return { tier: 'IGNITER', blurb: 'Early-stage cascade — tokens are largely burned for context, not yet compounding. The starting line.' }
}
  ...
  return (
    <div className="rounded-xl border border-bg-border bg-bg-surface p-6">
      ...
      <p className="mt-1 font-sans text-sm leading-relaxed text-text-secondary">
        Pull these from <code className="font-mono text-text-primary">ccusage --json</code> or{' '}
        <code className="font-mono text-text-primary">npx sigrank me</code>. Token counts only — no prompt content.
      </p>
      ...
      <div className="mt-6 rounded-lg border border-bg-border bg-bg-elevated p-5">
        <div className="flex flex-wrap items-baseline justify-between gap-2">
          <span className="font-mono text-xs uppercase tracking-widest text-text-muted">Υ Yield</span>
          <span className="font-mono text-3xl font-bold text-gold">{fmt(result.yield_)}</span>
        </div>
        <div className="mt-3 flex flex-wrap gap-x-6 gap-y-1 font-mono text-xs text-text-secondary">
          <span>Compression: <span className="text-text-primary">{result.compression.toFixed(2)}</span></span>
          <span>Cache hit: <span className="text-text-primary">{(result.cacheHitRate * 100).toFixed(0)}%</span></span>
          <span>Leverage: <span className="text-text-primary">{result.leverage.toFixed(1)}x</span></span>
        </div>
        <div className="mt-4 flex flex-col gap-1">
          <span className="font-mono text-sm font-bold text-accent">{result.tier}</span>
          <p className="font-sans text-sm leading-relaxed text-text-secondary">{result.blurb}</p>
        </div>
      </div>

      <p className="mt-4 font-sans text-xs leading-relaxed text-text-muted">
        Class thresholds are approximate (IGNITER &lt; 0.5, SEEKER 0.5–2, BUILDER 2–10, TRANSMITTER 10+).
        For the full paste-based scoring experience with signed submission, see{' '}
        <a href="/score" className="text-text-accent underline-offset-2 hover:underline">/score</a>.
      </p>
    </div>
  )
}
