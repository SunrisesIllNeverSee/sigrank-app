    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-8 py-2">
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          <strong className="text-text-primary">Υ = cache_read × output / input²</strong>.
          Yield rewards two things at once: reusing cached context (cache_read) and producing
          dense output per fresh token (output / input). An operator who re-sends the same
          large context every turn burns input without compounding — low yield. An operator
          who caches aggressively and extracts high signal per fresh token compounds — high
          yield. Volume is noise; yield is signal.
        </p>
        <p className="font-sans text-sm leading-relaxed text-text-secondary">
          The four pillars map directly to the metrics most AI platforms now expose:
          input (fresh prompt tokens), output (generated tokens), cache-read (prompt-caching
          hits), and cache-write (new tokens written to cache for future reuse).
        </p>
      </section>
