...
const DEFAULT_A: Cascade = {
  input: "12000",
  output: "4500",
  cacheRead: "80000",
  cacheWrite: "15000",
};
const DEFAULT_B: Cascade = {
  input: "30000",
  output: "2000",
  cacheRead: "10000",
  cacheWrite: "5000",
...
function metrics(c: Cascade) {
  ...
  const cacheHitRate =
    cacheRead + cacheWrite > 0 ? cacheRead / (cacheRead + cacheWrite) : 0;
...
const ROWS: {
  label: string;
  key: "yield_" | "compression" | "cacheHitRate" | "leverage";
  higher: boolean;
  fmt: (n: number) => string;
}[] = [
  ...
  return (
    <div className="rounded-xl border border-bg-border bg-bg-surface p-6">
            ...
            {ROWS.map((row) => {
              ...
              return (
                <tr key={row.key} className="border-t border-bg-border-subtle">
                  ...
                  <td
                    className={`px-4 py-2 text-right font-mono ${aWins ? "text-gold" : "text-text-primary"}`}
                  >
                    {row.fmt(va)}
                  </td>
                  <td
                    className={`px-4 py-2 text-right font-mono ${bWins ? "text-gold" : "text-text-primary"}`}
                  >
                    {row.fmt(vb)}
                  </td>
      ...
      <p className="mt-4 font-sans text-xs leading-relaxed text-text-muted">
        Comparing raw cascades is a quick diagnostic. For the full
        operator-vs-operator experience (radars, history, signed snapshots), see{" "}
        <a
          href="/compare"
