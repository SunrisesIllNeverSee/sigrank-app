...
function aggregateBenford(values, minN = 30, threshold = 15.5) {
  ...
  if (n < minN) {
    return { n, observed: [], expected: [], chiSquared: 0, flagged: false,
  ...
  let chiSquared = 0
  for (let i = 0; i < 9; i++) {
    if (expected[i] > 0) chiSquared += Math.pow(observed[i] - expected[i], 2) / expected[i]
  }
  return { n, observed, expected, chiSquared, flagged: chiSquared > threshold,
    summary: chiSquared > threshold
...
function checkBounds(rt) {
  ...
  if (rt.tokens_cache_creation > 0 && rt.tokens_cache_read / rt.tokens_cache_creation > 35) {
    issues.push({ code: 'extreme_cache_ratio' })
  }
  if (rt.tokens_output > 1_000 && rt.tokens_cache_creation / rt.tokens_output < 0.5) {
  ...
  if (rt.active_minutes_est > 0 && rt.turns_total / rt.active_minutes_est > 15) {
...
test('OLD bounds: tuned fabricator passes (the hole)', () => {
  ...
  // Υ is absurdly high:
  const upsilon = tunedFab.tokens_cache_read * tunedFab.tokens_output / (tunedFab.tokens_input_fresh ** 2)
...
test('NEW bounds: tuned fabricator is flagged', () => {
  ...
  assert.ok(issues.length >= 3, `at least 3 flags (got ${issues.length}: ${issues.map(i => i.code).join(', ')})`)
  assert.ok(issues.some(i => i.code === 'extreme_cache_ratio'), 'reuse 100:1 > 35:1')
  assert.ok(issues.some(i => i.code === 'low_cache_write_ratio'), 'cacheWrite 0.01 < 0.5')
  assert.ok(issues.some(i => i.code === 'implausible_input_share'), 'input 0.005% < 0.1%')
})

test('NEW bounds: real data (MO§ES) passes clean', () => {
  const issues = checkBounds(realData)
  assert.equal(issues.length, 0, `real data should pass clean (got: ${issues.map(i => i.code).join(', ')})`)
})
...
test('aggregate Benford: real data follows Benford (no flag)', () => {
  ...
  const realValues = [
    1_251_211, 11_296_121, 128_196_310, 2_555_179_769,  // MO§ES
    500_000, 1_200_000, 3_000_000, 80_000_000,          // operator 2
    200_000, 800_000, 1_500_000, 40_000_000,            // operator 3
    1_800_000, 9_000_000, 95_000_000, 1_800_000_000,    // operator 4
    300_000, 600_000, 2_200_000, 55_000_000,            // operator 5
    900_000, 4_500_000, 12_000_000, 300_000_000,        // operator 6
    150_000, 700_000, 1_800_000, 22_000_000,            // operator 7
    2_100_000, 15_000_000, 180_000_000, 3_200_000_000,  // operator 8
    400_000, 1_100_000, 4_500_000, 90_000_000,          // operator 9
    75_000, 350_000, 900_000, 18_000_000,               // operator 10
  ]
  const result = aggregateBenford(realValues, 30, 15.5)
  assert.ok(result.n >= 30, `n=${result.n} >= 30`)
  assert.equal(result.flagged, false, `real data should not flag (χ²=${result.chiSquared.toFixed(1)})`)
})
...
test('aggregate Benford: fabricated data diverges (flagged)', () => {
  ...
  assert.ok(result.flagged, `fabricated data should flag (χ²=${result.chiSquared.toFixed(1)})`)
  assert.ok(result.chiSquared > 15.5, `χ²=${result.chiSquared.toFixed(1)} > 15.5`)
})
