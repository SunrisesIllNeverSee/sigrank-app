...
test("MO§ES canonical: SNR (compression_ratio)", () => {
  ...
  assert.ok(
    Math.abs(m.snr - 0.9002) < 0.001,
    `snr=${m.snr.toFixed(4)} expected ~0.9002`,
  );
});
...
test("MO§ES canonical: leverage", () => {
  ...
  const m = compute(i, o, cw, cr);
  assert.ok(
    Math.abs(m.leverage - 2042.2) < 1,
    `leverage=${m.leverage.toFixed(1)} expected ~2042.2`,
  );
});
...
test("MO§ES canonical: yield", () => {
  ...
  const m = compute(i, o, cw, cr);
  assert.ok(
    Math.abs(m.yield_ - 18436.98) < 0.1,
    `yield=${m.yield_.toFixed(2)} expected ~18436.98`,
  );
});
...
test("MO§ES canonical: dev10x", () => {
  ...
  assert.ok(
    Math.abs(m.dev10x - 3.31) < 0.01,
    `dev10x=${m.dev10x.toFixed(2)} expected ~3.31`,
  );
});
...
test("MO§ES bridge: compression_ratio in [0,1]", () => {
  ...
  assert.ok(
    core5.compression_ratio >= 0 && core5.compression_ratio <= 1,
    `compression_ratio=${core5.compression_ratio} out of [0,1]`,
  );
  assert.ok(
    Math.abs(core5.compression_ratio - 0.9002) < 0.001,
...
test("MO§ES bridge: cross_thread in [0,100]", () => {
  ...
  const core5 = pillarsToCore5(i, o, cw, cr, 100, 5000);
  assert.ok(
    core5.cross_thread >= 0 && core5.cross_thread <= 100,
...
test("Codex Beta pathway (with io_ratio=9.03): est_input = output × io_ratio", () => {
  ...
  assert.ok(
    Math.abs(estInput - 1_251_211) < 2,
    `estInput=${estInput} expected ~1_251_211`,
  );
});
