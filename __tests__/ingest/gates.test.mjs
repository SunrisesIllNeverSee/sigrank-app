...
function plausibilityGate(p) {
  ...
  if (
    ...
    rt.turns_total / rt.active_minutes_est > 50
  ) {
...
function cleanPayload(over = {}) {
  ...
  rt.tokens_total =
    over.raw_telemetry?.tokens_total ??
    rt.tokens_input_fresh +
      rt.tokens_output +
      rt.tokens_cache_read +
      rt.tokens_cache_creation;
...
test("fabricated tokens_total (≠ Σ pillars) → reject totals_inconsistent", () => {
  const r = plausibilityGate(
    cleanPayload({ raw_telemetry: { tokens_total: 999_999_999_999 } }),
  );
...
test("tokens with zero sessions → reject tokens_without_sessions", () => {
  const r = plausibilityGate(
    cleanPayload({ raw_telemetry: { sessions_count: 0 } }),
  );
...
test("turns_total < sessions_count → reject turns_lt_sessions", () => {
  const r = plausibilityGate(
    cleanPayload({ raw_telemetry: { sessions_count: 40, turns_total: 10 } }),
  );
...
test("output with zero turns → reject output_without_turns", () => {
  const r = plausibilityGate(
    cleanPayload({ raw_telemetry: { sessions_count: 0, turns_total: 0 } }),
  );
...
test("implausible output rate (huge output, tiny active minutes) → flag", () => {
  const r = plausibilityGate(
    cleanPayload({
      raw_telemetry: { active_minutes_est: 5, tokens_output: 11_296_121 },
    }),
  );
...
test("cache_read > 1000 with cache_creation = 0 → flag cache_without_creation", () => {
  const r = plausibilityGate(
    cleanPayload({
      raw_telemetry: { tokens_cache_read: 50_000, tokens_cache_creation: 0 },
    }),
  );
...
test("cache_read/cache_creation > 100:1 → flag extreme_cache_ratio", () => {
  const r = plausibilityGate(
    cleanPayload({
      raw_telemetry: { tokens_cache_read: 500_000, tokens_cache_creation: 100 },
    }),
  );
...
test("turns/active_minutes > 50 → flag implausible_cadence", () => {
  const r = plausibilityGate(
    cleanPayload({
      raw_telemetry: { turns_total: 10_000, active_minutes_est: 10 },
    }),
  );
