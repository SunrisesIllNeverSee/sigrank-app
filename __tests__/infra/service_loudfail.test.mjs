/**
 * __tests__/infra/service_loudfail.test.mjs — pre-flip gate 2/3 (D7 §6.2).
 *
 ...
 *
 * Run: node --test __tests__/infra/service_loudfail.test.mjs
 */
import { test } from "node:test";
import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import { serviceKeyOrNull } from "../../lib/infra/supabase/service-config.mjs";
...
test("getSupabaseService uses serviceKeyOrNull and never the anon/active key", () => {
  const src = readFileSync(
    new URL("../../lib/infra/supabase/server.ts", import.meta.url),
    "utf-8",
  );
  ...
  assert.match(
    body,
    /serviceKeyOrNull\(/,
    "getSupabaseService delegates the key decision to serviceKeyOrNull",
  );
  assert.ok(
    !/activeKey|anonKey/.test(body),
    "getSupabaseService never references the anon/active key (no fallback)",
  );
});
