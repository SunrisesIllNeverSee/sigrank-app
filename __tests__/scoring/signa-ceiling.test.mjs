...
const signa = (s) =>
  Math.max(
    ...
    Math.min(
      100,
      W.comp * s.comp + W.sd * s.sd + W.pc * s.pc + W.ct * s.ct + W.tt * s.tt,
    ),
  );
...
test("max achievable SIGNA (all components = 100) is exactly 85", () => {
  ...
  assert.equal(
    max,
    85,
    "ceiling below 100 confirms the compressed class curve",
  );
});
...
test("TRANSMITTER (signaMin 85) requires a PERFECT operator", () => {
  ...
  assert.ok(
    almost < 85,
    `SIGNA=${almost}; anything less than perfect misses the 85 gate`,
  );
});
