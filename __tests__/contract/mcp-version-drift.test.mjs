...
function extractWebConstants(filePath) {
  ...
  if (!versionMatch) throw new Error(`Could not extract MCP_VERSION from ${filePath}`)
  const mcpVersion = versionMatch[1]

  const countMatch = src.match(/PLATFORM_COUNT\s*=\s*(\d+)/)
  if (!countMatch) throw new Error(`Could not extract PLATFORM_COUNT from ${filePath}`)
...
function extractMcpVersion(repoDir) {
  ...
  if (!pkg.version) throw new Error(`No version field in ${repoDir}/package.json`)
  return pkg.version
}
