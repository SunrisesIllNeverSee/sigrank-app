...
const parseSnapshotDate = (d) =>
  d ? Date.parse(`${d.slice(0, 10)}T00:00:00Z`) : NaN;
...
const day = (n) =>
  new Date(Date.UTC(2026, 5, 1) - n * MS_PER_DAY).toISOString().slice(0, 10); // n days before 2026-06-01
...
test("exact: a 30d snapshot is NOT in the 7d window", () => {
  assert.equal(
    inWindow(w("7d"), { window_type: "30d", snapshot_date: day(0) }, REF),
    false,
  );
});
test("exact: a 7d snapshot is in the 7d window", () => {
  assert.equal(
    inWindow(w("7d"), { window_type: "7d", snapshot_date: day(0) }, REF),
    true,
  );
});
...
test("7d buffer: 8 days old (7+1) is included; 9 days old is dropped", () => {
  assert.equal(
    inWindow(w("7d"), { window_type: "7d", snapshot_date: day(8) }, REF),
    true,
  );
  assert.equal(
    inWindow(w("7d"), { window_type: "7d", snapshot_date: day(9) }, REF),
    false,
  );
});
test("30d buffer: 33 days old (30+3) is included; 34 days old is dropped", () => {
  assert.equal(
    inWindow(w("30d"), { window_type: "30d", snapshot_date: day(33) }, REF),
    true,
  );
  assert.equal(
    inWindow(w("30d"), { window_type: "30d", snapshot_date: day(34) }, REF),
    false,
  );
});
test("90d buffer: 93 days old (90+3) is included; 94 days old is dropped", () => {
  assert.equal(
    inWindow(w("90d"), { window_type: "90d", snapshot_date: day(93) }, REF),
    true,
  );
  assert.equal(
    inWindow(w("90d"), { window_type: "90d", snapshot_date: day(94) }, REF),
    false,
  );
});
...
test("all_time: a 1000-day-old all_time snapshot still counts", () => {
  assert.equal(
    inWindow(
      w("all_time"),
      { window_type: "all_time", snapshot_date: day(1000) },
      REF,
    ),
    true,
  );
...
test("all_time: a 30d snapshot is NOT in all_time (exact window_type)", () => {
  assert.equal(
    inWindow(w("all_time"), { window_type: "30d", snapshot_date: day(0) }, REF),
    false,
  );
});
