...
-- A plpgsql function body is a single tx; SELECT ... FOR UPDATE on the code row
-- serializes concurrent redeems of the same single-use code (closes the
-- check-then-write double-enroll race that a supabase-js sequence cannot).
--
-- Depends on: device_enroll_codes (0013), devices (0001), operators (0001).
-- Additive + re-runnable (CREATE OR REPLACE). Same security posture as 0013:
-- search_path pinned '' + schema-qualified refs; EXECUTE locked to service_role.
--
-- Kept OUT of 0013 on purpose: 0013 is under review / pending owner-apply, so this
-- new object lands as its own migration (clean coordination + migration hygiene).
--
-- Returns a single row { status, operator_id, codename } — status drives the route's
-- HTTP mapping (no exception-for-control-flow):
--   'code_invalid'            → 410   (unknown / expired / already-consumed — no oracle)
--   'device_already_enrolled' → 409   (immutable binding; no rebind / rotation this slice)
--   'enrolled'                → 201
-- operator_id is taken FROM THE CODE ROW ONLY — never from a payload or codename (§5.4).
-- ============================================================================
...
BEGIN
  ...
  -- Immutable binding: a device_id enrolls exactly once (no rebind / no key rotation this slice).
  ...
  -- Bind the device to the CODE's operator — trusted immediately (web-auth'd single-use code §4.3).
...
-- Lock EXECUTE to service_role (Postgres auto-grants PUBLIC → PostgREST anon RPC surface).
