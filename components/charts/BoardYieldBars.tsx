...
const SPECIES: Record<string, string> = { casc: T.casc, arch: T.arch, power: T.power, base: T.base }
...
export default function BoardYieldBars({ entries, label }: BoardYieldBarsProps) {
  ...
  if (ranked.length === 0) {
    return (
      ...
      >
        Υ YIELD · {label.toUpperCase()} — no compounding operators in this window yet.
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: 1180, margin: '0 auto', width: '100%' }}>
      ...
      >
        <text x={padL} y={22} fontSize={12} fill={T.gold} style={{ letterSpacing: '.13em' }}>
          Υ YIELD · {label.toUpperCase()}
        </text>
        <text x={width - padL} y={22} fontSize={11} fill={T.mut} textAnchor="end">
        ...
        {ranked.map((e, i) => {
          ...
          return (
            <g key={`${e.anonId}-${i}`}>
              {/* rank + operator name */}
              <text x={padL} y={cy} dominantBaseline="middle" fontSize={11} fill={T.mut}>
                {e.rank}
              </text>
              <text
                x={padL + 22}
              ...
              {/* track + bar */}
              <rect x={plotL} y={y + 7} width={plotW} height={rowH - 14} fill={T.rowLine} rx={2} />
              <rect x={plotL} y={y + 7} width={w} height={rowH - 14} fill={SPECIES[sp]} rx={2} />
              {/* value */}
              <text
                x={width - padL}
                y={cy}
