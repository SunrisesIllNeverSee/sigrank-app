...
export default function CascadeRadar({ values, axes, series, size = 320 }: CascadeRadarProps) {
  ...
  const labels: string[] = multi ? axes!.map((a) => a.label) : (values ?? []).map((a) => a.label)
  const maxes: number[] = multi ? axes!.map((a) => a.max) : (values ?? []).map((a) => a.max)
  ...
  return (
    <svg
      ...
      width="100%"
      role="img"
      aria-label={multi ? 'Operator comparison radar' : 'Cascade fingerprint radar'}
      className="font-sans"
    ...
    >
      ...
      {rings.map((ring) => (
        <polygon
          key={ring}
          points={labels
            .map((_, i) => point(i, ring * radius).map((v) => v.toFixed(2)).join(','))
            .join(' ')}
          fill="none"
          stroke={LINE}
          strokeWidth={1}
      ...
      {polys.map((s, si) =>
        s.variant === 'ghost'
          ? null
          : labels.map((_, i) => {
              const [x, y] = point(i, norm(s.values[i] ?? 0, maxes[i]) * radius)
        ...
        polys
          ...
          .map((s, si) => (
            <g key={`leg-${si}`}>
              ...
              <text x={22} y={17 + si * 16} fill={INK} fontSize={10} className="font-sans">
                {s.name}
              </text>
            </g>
          ))}
