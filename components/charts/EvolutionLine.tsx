...
export default function EvolutionLine({
  ...
  return (
    ...
    >
      {/* title */}
      <text x={padL} y={14} fontSize={11} fill={MUTED} style={{ letterSpacing: '0.04em' }}>
        {label.toUpperCase()}
      </text>
      ...
      {gridLines.map((t) => {
        ...
        return (
          <g key={t}>
            <line x1={padL} y1={y.toFixed(2)} x2={width - padR} y2={y.toFixed(2)} stroke={LINE} strokeWidth={1} opacity={0.6} />
            <text
              x={padL - 6}
              y={y.toFixed(2)}
              textAnchor="end"
      ...
      {/* last-point dot + halo */}
      <circle cx={lastX.toFixed(2)} cy={lastY.toFixed(2)} r={5} fill={GOLD} fillOpacity={0.2} />
      <circle cx={lastX.toFixed(2)} cy={lastY.toFixed(2)} r={3} fill={GOLD} />

      {/* x-axis end labels */}
      <text x={padL} y={height - 8} textAnchor="start" fontSize={10} fill={MUTED}>
        {points[0].date}
      </text>
      {n > 1 && (
        <text x={width - padR} y={height - 8} textAnchor="end" fontSize={10} fill={BONE}>
          {last.date}
        </text>
      )}
    </svg>
