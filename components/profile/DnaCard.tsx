...
const ARCHETYPES: { name: string; condition: (dist: Record<string, number>) => boolean; meaning: string }[] = [
  { name: 'Sustained Burner', condition: (d) => (d.MAINTAIN ?? 0) > 0.7, meaning: 'Lives in the cascade. Efficient but may not start new things often.' },
  { name: 'Greenfield Specialist', condition: (d) => (d.BUILD ?? 0) > 0.5, meaning: 'Starts lots of things. May struggle to reach compounding.' },
  { name: 'Editor', condition: (d) => (d.EDIT ?? 0) > 0.5, meaning: 'Polishing-focused. High output, moderate reuse.' },
  { name: 'Debugger', condition: (d) => (d.DEBUG ?? 0) > 0.3, meaning: 'Investigative. High input, low output. May be stuck.' },
  { name: 'Cycle Master', condition: (d) => {
    const b = d.BUILD ?? 0, e = d.EDIT ?? 0, m = d.MAINTAIN ?? 0
    return b > 0.15 && e > 0.15 && m > 0.15 && b + e + m > 0.6
  }, meaning: 'Full project arcs. Starts, finishes, repeats.' },
]

function classifyArchetype(dist: Record<string, number>): { name: string; meaning: string } {
    ...
    <div className="rounded-lg border border-bg-border p-4">
      ...
      <div className="mt-3 flex flex-col gap-2 font-mono text-sm">
        ...
        <div>
          ...
          <span className="text-text-secondary">{earnedCount} earned, {inProgressCount} in progress</span>
        </div>
