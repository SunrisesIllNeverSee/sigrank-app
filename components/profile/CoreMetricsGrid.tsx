...
function valueFor(key: string, snapshot: ScoredSnapshot, t?: TelemetryRaw | null): string {
  const c = snapshot.cascade
  switch (key) {
    // RAW pillars (from telemetry)
    case 'input': return t ? fmtBig(t.fresh_input) : '—'
    case 'output': return t ? fmtBig(t.output) : '—'
    case 'cacheRead': return t ? fmtBig(t.cache_read) : '—'
    case 'cacheWrite': return t ? fmtBig(t.cache_create) : '—'
    case 'totalTokens':
      return t ? fmtBig(t.fresh_input + t.output + t.cache_read + t.cache_create) : '—'
    // Cascade metrics (derived)
    case 'yield_': return c && !c.nonCompounding ? fmtBig(c.yield_) : '—'
    case 'snr': return c ? `${(c.snr * 100).toFixed(1)}%` : '—'
    case 'leverage': return c && !c.nonCompounding ? `${fmtBig(c.leverage)}×` : '—'
    case 'velocity': return c ? c.velocity.toFixed(2) : '—'
    case 'dev10x': return c && !c.nonCompounding && c.dev10x !== null ? c.dev10x.toFixed(2) : '—'
    case 'scaleV': return c ? c.scaleV.toFixed(2) : '—'
    case 'costPerMillion': return c ? `$${c.costPerMillion.toFixed(2)}` : '—'
    case 'efficiency': return c ? c.efficiency.toFixed(2) : '—'
    case 'opRatio': return c ? c.opRatio : '—'
    default: return '—'
  }
}

function Card({ m, snapshot, telemetry }: { m: DisplayMetric; snapshot: ScoredSnapshot; telemetry?: TelemetryRaw | null }) {
  return (
    <div className="rounded-xl border border-bg-border-subtle bg-bg-surface p-4 transition-colors hover:border-bg-border">
      ...
      <div className="mb-1.5 text-[12px] font-medium text-text-muted">{m.name}</div>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <section className="flex flex-col gap-2">
        <span className="font-mono text-[11px] uppercase tracking-wider text-text-muted">Raw — the pillars</span>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {DISPLAY_RAW.map((m) => (
            <Card key={m.id + m.key} m={m} snapshot={snapshot} telemetry={telemetry} />
          ))}
        </div>
      </section>
      {/* METRICS group (9) */}
      <section className="flex flex-col gap-2">
        <span className="font-mono text-[11px] uppercase tracking-wider text-text-muted">Metrics — the cascade</span>
