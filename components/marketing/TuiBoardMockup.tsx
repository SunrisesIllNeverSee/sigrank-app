...
export function TuiBoardMockup({
  highlightYou = false,
}: {
    ...
    >
      ...
      <div className="overflow-x-auto px-4 py-3">
        <div className="font-mono text-[11px] text-text-dim whitespace-nowrap">
          Leaderboard&nbsp;&nbsp;window: 30d&nbsp;&nbsp;·&nbsp;&nbsp;sorted by Υ
          Yield
        </div>
        {/* column headers */}
        ...
        {ROWS.map((r) => {
          ...
          return (
            ...
            >
              <span
                className={`w-8 text-right ${r.rank <= 3 ? "font-bold text-gold" : "text-text-muted"}`}
              >
                #{r.rank}
              </span>
              <span
                className={`w-36 truncate ${showYou ? "font-bold text-gold" : "text-text-primary"}`}
              >
                {r.name}
                {showYou && <span className="ml-1 text-gold/70">YOU</span>}
              </span>
              ...
              <span
                className={`w-10 text-right ${r.mv === "▲" ? "text-green-400" : r.mv === "▼" ? "text-red-400" : "text-text-dim"}`}
              >
                {r.mv}
              </span>
            </div>
      ...
      <div className="border-t border-bg-border-subtle px-4 py-2 font-mono text-[10px] leading-snug text-text-dim">
        Illustrative — mirrors the live 30d board. See signalaf.com/board/30d
        for real-time data.
      </div>
    </div>
  );
