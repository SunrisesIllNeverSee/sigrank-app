...
function NoiseField({
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      const flicker =
        phase === PHASE_NOISE ? (Math.random() > 0.92 ? 0.3 : 1) : 1;
...
export function SignalSignature({
  ...
  useEffect(() => {
    ...
    function tick(now: number) {
      ...
      if (elapsed < NOISE_DURATION) {
        ...
        elapsed <
        NOISE_DURATION + RESOLVE_DURATION + SIGNAL_DURATION
      ) {
        setPhase(PHASE_SIGNAL);
        setProgress(1);
      } else if (
        elapsed <
        NOISE_DURATION + RESOLVE_DURATION + SIGNAL_DURATION + HOLD_DURATION
      ) {
        setPhase(PHASE_HOLD);
        setProgress(1);
      } else {
  ...
  const radarOpacity =
    phase === PHASE_NOISE ? 0 : phase === PHASE_RESOLVE ? progress : 1;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 30%, rgba(10,10,10,0.7) 80%, rgba(10,10,10,0.95) 100%)",
        }}
      />
