...
interface ScoreResponse {
  ...
  core5: { compression_ratio: number; prompt_complexity: number; cross_thread: number; session_depth: number; token_throughput: number }
...
const PILLAR_PRESETS: Record<string, { pillars: RawPillars; sessions: number; turns: number; label: string }> = {
  ...
  archplus: {
    label: 'ARCH+ operator',
    pillars: { input: 3_400_000, output: 5_600_000, cacheCreate: 68_000_000, cacheRead: 1_163_000_000 },
    sessions: 300, turns: 5_000,
  },
  power: {
    label: 'POWER operator',
    pillars: { input: 2_000_000, output: 1_500_000, cacheCreate: 15_000_000, cacheRead: 200_000_000 },
    sessions: 150, turns: 2_500,
  },
  base: {
    label: 'BASE operator',
    pillars: { input: 800_000, output: 400_000, cacheCreate: 3_000_000, cacheRead: 30_000_000 },
    sessions: 50, turns: 600,
  },
  burner: {
    label: 'BURNER (no cache)',
    pillars: { input: 500_000, output: 100_000, cacheCreate: 0, cacheRead: 0 },
    sessions: 20, turns: 200,
  },
}
  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex justify-between items-baseline">
        <label className="text-xs uppercase tracking-wider text-[rgb(var(--text-muted))]">{label}</label>
        <span className="text-sm font-mono text-[rgb(var(--text-primary))]" style={{ color: accent }}>
          {format(value)}
        </span>
      </div>
      <input
...
function MetricCard({
  ...
  return (
    <div className="flex flex-col gap-0.5 p-3 rounded-md border border-[rgb(var(--bg-border))] bg-[rgb(var(--bg-surface))]">
      <span className="text-[10px] uppercase tracking-wider text-[rgb(var(--text-muted))]">{label}</span>
      <span className="text-xl font-mono font-bold" style={{ color: accent ?? 'rgb(var(--text-primary))' }}>
        {value}
      </span>
      {sub && <span className="text-[10px] text-[rgb(var(--text-dim))]">{sub}</span>}
    </div>
  )
}
...
export function SandboxClient() {
  ...
  const exportJson = () => {
    const data = { pillars, sessions, turns, cascade, score, timestamp: new Date().toISOString() }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-6">
        ...
        <div className="flex flex-col gap-4 p-4 rounded-lg border border-[rgb(var(--bg-border))] bg-[rgb(var(--bg-surface))]">
          ...
          <Slider
            label="Fresh Input" value={pillars.input}
          ...
          <Slider
            ...
            onChange={(v) => { setSessions(v); setActivePreset('') }}
            min={1} max={1000} step={1}
            format={(v) => String(v)}
          />
          <Slider
            label="Turns" value={turns}
            onChange={(v) => { setTurns(v); setActivePreset('') }}
            min={1} max={20_000} step={10}
            format={(v) => String(v)}
          />
          <Slider
            label="Lifetime Messages" value={lifetime}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <MetricCard
              label="Υ Yield" value={fmt(cascade.yield_, { decimals: 2 })}
              sub={`lev ${fmt(cascade.leverage, { decimals: 1 })}× vel ${fmt(cascade.velocity, { decimals: 2 })}`}
              accent="rgb(var(--gold))"
            />
            <MetricCard
              label="SignaRate" value={score ? fmt(score.signa_rate, { decimals: 1 }) : '—'}
              sub={loading ? 'computing...' : 'server-scored'}
            />
            <MetricCard
              label="Class" value={score?.class_tier ?? '—'}
              sub={score ? `SF ${fmt(score.signal_force, { decimals: 1 })}` : ''}
              accent={classColor}
            />
            <MetricCard
              label="10xDEV" value={cascade.dev10x !== null ? fmt(cascade.dev10x, { decimals: 2 }) : '—'}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <MetricCard label="Scale V" value={fmt(cascade.scaleV, { decimals: 2 })} />
            <MetricCard label="$/1M" value={`$${fmt(cascade.costPerMillion, { decimals: 2 })}`} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {trajectoryData.length > 0 && (
              <div className="p-3 rounded-lg border border-[rgb(var(--bg-border))] bg-[rgb(var(--bg-surface))]">
                <h4 className="text-xs uppercase tracking-wider text-[rgb(var(--text-muted))] mb-2">Trajectory</h4>
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={trajectoryData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="rgb(var(--bg-border))" />
                    <XAxis dataKey="run" tick={{ fill: 'rgb(var(--text-muted))', fontSize: 10 }} />
                    <YAxis tick={{ fill: 'rgb(var(--text-muted))', fontSize: 10 }} />
                    <Tooltip
                      contentStyle={{
                    ...
                    <Line type="monotone" dataKey="Υ" stroke="rgb(var(--gold))" strokeWidth={2} dot={{ r: 3 }} />
                    <Line type="monotone" dataKey="Signa" stroke="rgb(var(--accent))" strokeWidth={2} dot={{ r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <div className="p-3 rounded-lg border border-[rgb(var(--bg-border))] bg-[rgb(var(--bg-surface))]">
            <h4 className="text-xs uppercase tracking-wider text-[rgb(var(--text-muted))] mb-2">Token Distribution</h4>
            <ResponsiveContainer width="100%" height={140}>
              <AreaChart data={[
                { name: 'Input', tokens: pillars.input, fill: 'rgb(var(--text-dim))' },
                { name: 'Output', tokens: pillars.output, fill: 'rgb(var(--accent))' },
                { name: 'Cache Create', tokens: pillars.cacheCreate, fill: 'rgb(var(--class-power))' },
                { name: 'Cache Read', tokens: pillars.cacheRead, fill: 'rgb(var(--gold))' },
              ]}>
                <XAxis dataKey="name" tick={{ fill: 'rgb(var(--text-muted))', fontSize: 10 }} />
                <YAxis tick={{ fill: 'rgb(var(--text-dim))', fontSize: 8 }} tickFormatter={(v) => fmt(v, { compact: true, decimals: 0 })} />
                <Tooltip
                  contentStyle={{
                    background: 'rgb(var(--bg-elevated))',
                    border: '1px solid rgb(var(--bg-border))',
                  ...
                  formatter={(v) => fmt(Number(v), { compact: true, decimals: 1 })}
                />
                <Area dataKey="tokens" stroke="rgb(var(--accent))" fill="rgb(var(--accent))" fillOpacity={0.2} strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg border border-[rgb(var(--bg-border))] bg-[rgb(var(--bg-surface))]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              ...
              <tbody>
                {history.map((h) => (
                  <tr key={h.id} className="border-b border-[rgb(var(--bg-border-subtle))]">
                    <td className="py-1 pr-3 text-[rgb(var(--text-secondary))]">{h.label}</td>
                    <td className="py-1 pr-3 text-[rgb(var(--gold))]">{fmt(h.cascade.yield_, { decimals: 2 })}</td>
                    <td className="py-1 pr-3">{fmt(h.signaRate, { decimals: 1 })}</td>
                    <td className="py-1 pr-3" style={{ color: CLASS_COLORS[h.classTier] }}>{h.classTier}</td>
                    <td className="py-1 pr-3">{fmt(h.cascade.leverage, { decimals: 1 })}×</td>
                    <td className="py-1 pr-3">{fmt(h.cascade.velocity, { decimals: 2 })}</td>
                    <td className="py-1 pr-3">{fmt(h.cascade.snr, { decimals: 3 })}</td>
                    <td className="py-1 pr-3">{h.cascade.dev10x !== null ? fmt(h.cascade.dev10x, { decimals: 2 }) : '—'}</td>
                    <td className="py-1 pr-3 text-[rgb(var(--text-dim))]">{h.timestamp}</td>
                  </tr>
                ))}
              </tbody>
            </table>
