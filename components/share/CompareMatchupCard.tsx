...
function CardRadar({
  ...
  const sorted = [...series]
    ...
    .sort(
      (p, q) =>
        (p.s.variant === "ghost" ? 0 : 1) - (q.s.variant === "ghost" ? 0 : 1),
    );
      ...
      {series.map((s, si) =>
        s.variant === "ghost"
          ...
          : axes.map((_, i) => {
              const [x, y] = point(
                i,
                norm(s.values[i] ?? 0, axes[i].max) * radius,
              );
              return (
                <circle
                  key={`pt-${si}-${i}`}
...
function Card({
  ...
  const rawRadarAxes = rAxes.map((ax) => ({
    label: `${ax.label} ${ax.higherWins ? "↑" : "↓"}`,
    max: ax.max,
  }));
  const metricRadarAxes = mAxes.map((ax) => ({
    label: `${ax.label} ${ax.higherWins ? "↑" : "↓"}`,
    max: ax.max,
  }));
  ...
  const rawSeries: CardRadarSeries[] = [
    {
      name: operatorDisplayName(a),
      values: rAxes.map((ax) => proj(ax, ax.a)),
      color: A_COLOR,
      variant: "solid",
    },
    {
      name: operatorDisplayName(b),
      values: rAxes.map((ax) => proj(ax, ax.b)),
      color: B_COLOR,
      variant: "solid",
    },
  ];
  const metricSeries: CardRadarSeries[] = [
    {
      name: `${operatorDisplayName(a)} raw`,
      values: fit(ghostA).map((v, i) => v * (mAxes[i]?.max ?? 1)),
      color: A_COLOR,
      variant: "ghost",
    },
    {
      name: `${operatorDisplayName(b)} raw`,
      values: fit(ghostB).map((v, i) => v * (mAxes[i]?.max ?? 1)),
      color: B_COLOR,
      variant: "ghost",
    },
    {
      name: operatorDisplayName(a),
      values: mAxes.map((ax) => proj(ax, ax.a)),
      color: A_COLOR,
      variant: "solid",
    },
    {
      name: operatorDisplayName(b),
      values: mAxes.map((ax) => proj(ax, ax.b)),
      color: B_COLOR,
      variant: "solid",
    },
  ];
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "stretch" }}>
          ...
          >
            <span style={{ fontSize: 32, color: colA, lineHeight: 1 }}>
              {glyphFor(clsA)}
            </span>
            <span
              style={{
          ...
          >
            <span style={{ fontSize: 32, color: colB, lineHeight: 1 }}>
              {glyphFor(clsB)}
            </span>
            <span
              style={{
      ...
      <div
        style={{
          ...
          justifyContent: "center",
          background:
            "linear-gradient(90deg, transparent, #1a3a1a 15%, #2a5a2a 50%, #1a3a1a 85%, transparent)",
          flexShrink: 0,
        }}
      >
        <div
      ...
      >
        ...
        >
          ...
          <span style={{ color: "#4a6a4a", fontSize: 9 }}>
            ↑ HIGHER BETTER · ↓ LOWER BETTER · GHOST = RAW FOOTPRINT
          </span>
        ...
        >
          ...
          >
            ...
            <div
              style={{
                transform:
                  "scale(min(1, calc((100vw - 2rem) / 1200), calc((100vh - 2rem) / 903)))",
