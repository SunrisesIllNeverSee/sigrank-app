...
function rawAxes(a: LeaderboardRow, b: LeaderboardRow): Axis[] {
  ...
  const mk = (label: string, av: number, bv: number, higherWins = true): Axis => ({
    label,
    a: av,
    b: bv,
...
function RadarBlock({
  ...
  const radarAxes = axes.map((ax) => ({ label: `${ax.label} ${ax.higherWins ? '↑' : '↓'}`, max: ax.max }))
  ...
  series.push({ name: aName, values: axes.map((ax) => proj(ax, ax.a)), color: A_COLOR, variant: 'solid' })
  series.push({ name: bName, values: axes.map((ax) => proj(ax, ax.b)), color: B_COLOR, variant: 'solid' })

  return (
    <div className="flex flex-col items-center gap-2 rounded-xl border border-bg-border bg-bg-surface p-4">
      <span className="font-sans text-[11px] uppercase tracking-widest text-text-muted">{title}</span>
      <CascadeRadar axes={radarAxes} series={series} size={300} />
      <span className="font-sans text-[10px] text-text-muted">
        ↑ higher is better · ↓ lower is better (cost) — both reach outward when good
      </span>
    </div>
  )
}

export function CompareRadars({ a, b }: { a: LeaderboardRow; b: LeaderboardRow }) {
