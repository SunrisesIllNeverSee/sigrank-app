  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-bg-border bg-bg-surface p-4">
      ...
      <div className="grid grid-cols-3 items-center gap-2 font-mono text-sm">
        <div
          className={`text-right ${aWon ? "text-gold font-bold" : "text-text-secondary"}`}
        >
          {dispA}
        ...
        <div
          className={`text-left ${bWon ? "text-gold font-bold" : "text-text-secondary"}`}
        >
          {dispB}
...
export function ChallengeBar({
  ...
  const [newChallenge, setNewChallenge] = useState<ActiveChallenge | null>(
    null,
  );
  ...
  if (challenge?.status === "complete") {
    return (
      <div className="flex flex-col gap-3">
        ...
        {status === "error" && (
          <p className="text-xs text-class-refiner">{errMsg}</p>
        )}
      </div>
    );
  }
