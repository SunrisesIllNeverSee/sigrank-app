...
interface Axis { key: string; label: string; canon: string; a: number; b: number; fmt: (n: number) => string }
...
function axesFor(a: LeaderboardRow, b: LeaderboardRow): Axis[] {
  ...
  const k = (n: number) => (n >= 1000 ? `${(n / 1000).toFixed(1)}K` : n.toFixed(n >= 100 ? 0 : 2))
  return [
    { key: 'yield', label: 'Υ Yield', canon: 'Y.01', a: comp(ca, (x) => x.yield_), b: comp(cb, (x) => x.yield_), fmt: k },
    { key: 'snr', label: 'SNR', canon: 'Y.02', a: raw(ca, (x) => x.snr), b: raw(cb, (x) => x.snr), fmt: (n) => n.toFixed(3) },
    { key: 'lev', label: 'Leverage', canon: 'Y.03', a: comp(ca, (x) => x.leverage), b: comp(cb, (x) => x.leverage), fmt: (n) => `${k(n)}×` },
    { key: 'vel', label: 'Velocity', canon: 'Y.04', a: raw(ca, (x) => x.velocity), b: raw(cb, (x) => x.velocity), fmt: (n) => n.toFixed(2) },
    { key: 'dev', label: '10xDEV', canon: 'Y.05', a: comp(ca, (x) => x.dev10x ?? 0), b: comp(cb, (x) => x.dev10x ?? 0), fmt: (n) => n.toFixed(2) },
    // cost: lower is better — handled in the lead logic below.
    { key: 'cost', label: '$/1M', canon: 'Y.07', a: raw(ca, (x) => x.costPerMillion), b: raw(cb, (x) => x.costPerMillion), fmt: (n) => `$${n.toFixed(2)}` },
  ]
}

export function CompareTitleCard({ a, b }: { a: LeaderboardRow; b: LeaderboardRow }) {
  ...
  const ranked = [...axes].filter((ax) => leads(ax) !== 'tie').sort((x, y) => gapOf(y) - gapOf(x))
        ...
        <div className="flex w-full max-w-md flex-col gap-2">
          ...
          <div className="flex flex-col gap-1.5">
            {highlights.map((ax) => {
              ...
              return (
                <div key={ax.key} className="flex items-center gap-3 font-mono text-[11px]">
                  <span className="w-24 shrink-0 text-text-secondary">
                    {ax.label}
                    <sup className="ml-0.5 text-[8px] text-text-dim">{ax.canon}</sup>
                  </span>
                  <span className={`shrink-0 tabular-nums ${w === 'a' ? 'font-bold text-gold' : 'text-text-dim'}`}>
                    {ax.fmt(ax.a)}
                  </span>
                  <span className="shrink-0 text-gold">{w === 'a' ? '◀' : '▶'}</span>
                  <span className={`shrink-0 tabular-nums ${w === 'b' ? 'font-bold text-gold' : 'text-text-dim'}`}>
                    {ax.fmt(ax.b)}
                  </span>
          ...
          return (
            <div key={ax.key} className="flex flex-col gap-1">
              ...
              <div className="flex items-center justify-between font-mono text-[11px]">
                <span className={w === 'a' ? 'font-bold text-text-primary' : 'text-text-dim'}>
                  {ax.fmt(ax.a)}
                </span>
                <span className={w === 'b' ? 'font-bold text-text-primary' : 'text-text-dim'}>
                  {ax.fmt(ax.b)}
                </span>
              </div>
              ...
              <div className="h-0.5 w-full overflow-hidden rounded-full bg-bg-border">
                <div
                  ...
                  style={{ opacity: w === 'tie' ? 0 : Math.min(1, 0.15 + gapOf(ax) * 0.85) }}
                />
              </div>
            </div>
          )
