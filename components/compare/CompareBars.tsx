...
function buildRows(a: LeaderboardRow, b: LeaderboardRow): MetricRow[] {
  ...
  return [
    { label: 'Υ Yield', canonId: 'Υ', a: yield_(ca), b: yield_(cb), higherWins: true, fmt: (v) => (v >= 1000 ? `${(v / 1000).toFixed(1)}K` : v.toFixed(0)), radarMax: Math.max(yield_(ca), yield_(cb), 1) },
    { label: 'SNR', canonId: 'M.01', a: snr(ca), b: snr(cb), higherWins: true, fmt: (v) => `${(v * 100).toFixed(1)}%`, radarMax: 1 },
    { label: 'Leverage', canonId: 'Cr/I', a: lev(ca), b: lev(cb), higherWins: true, fmt: (v) => (v >= 1000 ? `${(v / 1000).toFixed(1)}K×` : `${v.toFixed(0)}×`), radarMax: Math.max(lev(ca), lev(cb), 1) },
    { label: 'Velocity', canonId: 'O/I', a: vel(ca), b: vel(cb), higherWins: true, fmt: (v) => (v >= 10 ? v.toFixed(1) : v.toFixed(2)), radarMax: Math.max(vel(ca), vel(cb), 1) },
    { label: '10xDEV', canonId: '⚡', a: dev(ca), b: dev(cb), higherWins: true, fmt: (v) => v.toFixed(2), radarMax: 5 },
    { label: '$ / 1M', canonId: '$', a: cost(ca), b: cost(cb), higherWins: false, fmt: (v) => `$${v.toFixed(2)}`, radarMax: Math.max(cost(ca), cost(cb), 1) },
  ]
}
...
export function CompareBars({ a, b }: { a: LeaderboardRow; b: LeaderboardRow }) {
    ...
    <section className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between">
        ...
        <div className="flex items-center gap-4 font-mono text-[10px]">
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: A_COLOR }} />
            <span className="text-text-secondary">{aName}</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: B_COLOR }} />
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_240px]">
        ...
        {rows.map((r) => {
          ...
          const w = winnerOf(r)
          return (
            <div key={r.canonId} className="grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-1">
              {/* axis label + canon id */}
              ...
              <span className="text-right font-mono text-[11px] tabular-nums">
                <span className={w === 'a' ? 'text-text-accent' : 'text-text-muted'}>{r.fmt(r.a)}</span>
                <span className="mx-1 text-text-dim">·</span>
                <span className={w === 'b' ? 'text-class-seeker' : 'text-text-muted'}>{r.fmt(r.b)}</span>
              </span>
              ...
              <div className="col-span-2 flex h-2 overflow-hidden rounded-full bg-bg-base">
                <div className="flex h-full grow justify-end">
                  <div
                    ...
                    style={{ width: `${aPct}%`, background: A_COLOR, opacity: w === 'b' ? 0.45 : 1 }}
                  />
                </div>
                ...
                <div className="flex h-full grow">
                  <div
                    ...
                    style={{ width: `${bPct}%`, background: B_COLOR, opacity: w === 'a' ? 0.45 : 1 }}
                  />
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-col items-center gap-2 rounded-lg border border-bg-border bg-bg-surface p-3">
          <span className="font-sans text-[11px] uppercase tracking-wide text-text-muted">Shape</span>
