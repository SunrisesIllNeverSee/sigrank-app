...
const FIELDS: MetricField[] = [
  {
    key: "codename",
    label: "Codename",
    hint: "Your anonymous operator handle",
    placeholder: "e.g. SignalRunner",
  },
  {
    key: "output_tokens",
    label: "Output tokens",
    hint: "Total output tokens this window",
    placeholder: "1200000",
  },
  {
    key: "fresh_input_tokens",
    label: "Fresh input tokens",
    hint: "Non-cached input tokens",
    placeholder: "90000",
  },
  {
    key: "total_tokens",
    label: "Total tokens",
    hint: "All tokens this window",
    placeholder: "4200000",
  },
  {
    key: "sessions_count",
  ...
  },
  {
    key: "turns_total",
    label: "Turns",
    hint: "Total reply turns",
    placeholder: "4180",
  },
  {
    key: "message_volume",
    label: "Turn volume",
    hint: "Turns this window",
    placeholder: "2300",
  },
  {
    key: "account_age_days",
    label: "Account age (days)",
    hint: "Lifetime account age",
    placeholder: "64",
  },
  {
    key: "total_messages_lifetime",
    label: "Total turns (lifetime)",
    hint: "All-time turns",
    placeholder: "27600",
  },
];
...
export function SubmitForm() {
    ...
    try {
      ...
      if (res.ok) {
        setStatus({
          kind: "ok",
          detail:
            "Snapshot received. Manual entries are scored at reduced confidence.",
        });
      } else {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-xl border border-bg-border-subtle bg-bg-surface p-6"
    >
      <div className="mb-4 rounded-lg border border-gold/25 bg-gold/5 px-4 py-3 text-sm text-text-secondary">
        ...
        <span className="font-mono text-text-primary">confidence: low</span>.
        For exact cascade metrics, run the CLI agent or upgrade to Pro.
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        {FIELDS.map((f) => (
          <label key={f.key} className="flex flex-col gap-1">
            ...
            />
            <span id={`hint-${f.key}`} className="text-[11px] text-text-muted">
              {f.hint}
            </span>
          </label>
        ))}
