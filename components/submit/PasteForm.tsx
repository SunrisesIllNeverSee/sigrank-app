...
export function PasteForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setStatus({ kind: 'error', detail: 'Could not reach the submission endpoint.' })
    }
  }
      ...
      <div className="flex flex-col gap-5 rounded-xl border border-gold/30 bg-gold/5 p-6">
        ...
        <div className="rounded-lg border border-text-accent/25 bg-text-accent/5 px-4 py-3">
          ...
          <p className="mt-1 font-sans text-[12px] leading-snug text-text-secondary">
            Pasting computes your cascade so you can see where you&apos;d land. It does
            <strong className="text-text-primary"> not</strong> publish to the leaderboard.
            To claim a real spot, create an account and submit through the local agent —
            board entries are reviewed, so the board stays honest.
          </p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-lg border border-text-accent/25 bg-text-accent/5 px-4 py-3 text-sm text-text-secondary">
        <strong className="font-semibold text-text-accent">Run your numbers:</strong>{' '}
        Paste your ccusage export to see your cascade and projected rank instantly.
        Real token counts in — full breakdown out.{' '}
        <strong className="text-text-primary">This does not save to the leaderboard</strong>{' '}
        — it&apos;s a calculator. To land on the board, run the local agent (it reads your
        tokens automatically) and submit through your account.
      </div>
      ...
      <label className="flex flex-col gap-1">
        ...
        <textarea
          ...
          // enforced explicitly here, not left to PostHog's default masking. (2026-06-30)
      ...
      {preview && (
        <div className="rounded-lg border border-bg-border bg-bg-surface px-4 py-4">
          ...
          <form onSubmit={onSubmit} className="mt-4 flex flex-wrap items-end gap-4">
            <label className="flex flex-col gap-1">
              <span className="text-xs font-medium text-text-secondary">Codename <span className="text-text-dim">(optional)</span></span>
              <input
                type="text"
                value={codename}
            ...
            <label className="flex flex-col gap-1">
              <span className="text-xs font-medium text-text-secondary">Window (days)</span>
              <select
                value={windowLabel}
