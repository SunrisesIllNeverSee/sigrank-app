...
if (isJson) {
  ...
  // Dedupe by handle — keep the first occurrence (highest rank = most tokens)
  const seenHandles = new Set();
  const beforeDedupe = parsedRows.length;
  parsedRows = parsedRows.filter((r) => {
    if (seenHandles.has(r.handle)) return false;
    seenHandles.add(r.handle);
    return true;
  });
  if (parsedRows.length < beforeDedupe)
    console.log(`Deduped: ${beforeDedupe} → ${parsedRows.length} (removed ${beforeDedupe - parsedRows.length} duplicate handles)`);
